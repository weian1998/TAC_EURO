...
public partial class ImportLeadTemplates : EntityBoundSmartPartInfoProvider
{
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            if ((grdTemplatesDeleteColumnIndex >= 0) && (grdTemplatesDeleteColumnIndex < e.Row.Cells.Count))
            {
                ...
                foreach (Control c in cell.Controls)
                {
                    ...
                    if (btn != null)
                    {
                        btn.Attributes.Add("onclick", "javascript: return confirm('" + GetLocalResourceObject("grdTemplates.DeleteConfirmation") + "');");
                        return;
                    }
                }
            }
    ...
    public override Sage.Platform.Application.UI.ISmartPartInfo GetSmartPartInfo(Type smartPartInfoType)
    {
        ...
        foreach (Control c in pnlImportLeadTemplate_LTools.Controls)
        {
            tinfo.LeftTools.Add(c);
        }
        foreach (Control c in pnlImportLeadTemplate_CTools.Controls)
        {
            tinfo.CenterTools.Add(c);
        }
        foreach (Control c in pnlImportLeadTemplate_RTools.Controls)
        {
