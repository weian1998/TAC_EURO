...
function OnActionClick(action) {
    ...
    $('input[id$=_NonRecurActivityIds]').val(Reminders.getSelectedNonRecurActivities());
...
var Reminders = {
    ...
    getSelectedNonRecurActivities: function(allowRecur) {
        sel = [];
        $("input[id$=_chkRowSelect][data-recurring!='True']:checked").each(
            function() {
                sel.push($(this).attr("data-activityId"));
            }
        );
    ...
    getRecurringCount: function() {
        return $("input[id$=_chkRowSelect][data-recurring='True']").size();
    },
    ...
    getSelectedRecurringCount: function() {
        return $("input[id$=_chkRowSelect][data-recurring='True']:checked").size();
    },
    ...
    checkRecurring: function(fn) {   
        if (   (this.getSelectedRecurringCount() === 0)
            || (this.getRecurringCount() === 0)
           ) {
            fn(true);
            return;
        }       
        ...
        //Check cookie whether to display prompt
        var dontShow = cookie.getCookieParm("NotAllowedOnRecurringPrompt", "SLXReminderPreferences");        if (dontShow == "true")  
            {fn(true);
            return;}   

       this.reminderPreferencesPrompt(SR.NotAllowedOnRecurring, "NotAllowedOnRecurringPrompt", fn);
    ...
    completeSelected: function(ignoreConfirmations) {
        ...
        Reminders.checkRecurring(Reminders.doCompleteSelected);        
    },
    
    doCompleteSelected: function(){    
            if (Reminders.getSelectedRecurringCount() == Reminders.getSelectedCount())
            return;
...
<div style="display:none">
    ...
    <asp:Panel ID="ActivityReminders_CTools" runat="server">
        <asp:Button runat="server" ID="btnShowHideDetail" CssClass="slxbutton" style="padding-left:5px;padding-right:5px;" />
    </asp:Panel>
    ...
    <asp:HiddenField ID="NonRecurActivityIds" runat="server" />
        ...
        <Columns>
           ...
           <asp:TemplateField SortExpression="Recurring" HeaderText="<%$ Resources:Reminders_Grid_Header_Recur.HeaderText %>">
            ...
            <asp:TemplateField SortExpression="ActivityType" HeaderText="<%$ Resources:Reminders_Grid_Header_ActivityType.HeaderText %>">
                <itemtemplate>
                    <asp:HyperLink ID="lnkActivity" runat="server" Target="_top" Font-Bold="false" NavigateUrl='<%# BuildActivityNavigateUrl(Eval("ActivityId"), Eval("ReminderType"), Eval("Recurring"), Eval("StartDate")) %>'>
...
<tr>
    <td style="padding-left:40px">
        ...
        <asp:Image ID="imgRecurImage" ImageUrl="~/images/clear.gif" runat="server" />
    </td>
    <td align="right">
        <div runat="server" id="divActivityOptions" style="display:none">
            <asp:Image ID="imgComplete" ImageUrl="~/images/icons/Clear_Activity_16x16.gif" runat="server" />
