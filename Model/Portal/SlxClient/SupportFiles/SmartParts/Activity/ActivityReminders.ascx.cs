...
public partial class SmartParts_Activity_ActivityReminders : UserControl, ISmartPartInfoProvider
{
    ...
    private static readonly Dictionary<bool, string> RecurImageUrls =
    new Dictionary<bool, string>
        {
            {true, "images/icons/Recurring_16x16.gif"},
            {false, "images/clear.gif"}
        };
    ...
    private void CompleteSelectedIndividually()
    {
        ...
        var context = ApplicationContext.Current.Services.Get<IContextService>(true);
        var ids = GetListNonRecurActivityIds().ToArray();

        if (ids.Length > 1)
        {
            context.SetContext("CompleteActivityIds", GetListNonRecurActivityIds().ToList());
            args.Add("mode", "batch");
        }
    ...
    private void DeleteSelected()
    {
        foreach (var activity in ListActionActivities)
        {
            //Do not allow deletion of recurring activities from the Reminder view.
            if (!activity.Recurring)
            {
                activity.Delete();
                if (activity == _CurrentActivity) _CurrentActivity = null;
    ...
    private DataTable ResultsToDataSet(IEnumerable<Activity> results)
    {
        ...
        foreach (Activity activity in results)
        {
            ...
            row["RecurImage"] = Page.ResolveUrl(RecurImageUrls[activity.Recurring]);
            ...
            row["Recurring"] = activity.Recurring;
    ...
    private static DataTable CreateDataTable()
    {
        ...
        dt.Columns.Add("RecurImage");
        return dt;
    }

    protected string BuildActivityNavigateUrl(object activityId, object reminderType, object recurring, object startTime)
    {
        if (reminderType.Equals(SR.ReminderType_Confirm))
            return String.Format("javascript:Link.confirmActivity('{0}');", activityId);
        var retVal = ((string)recurring=="True") ?
              string.Format("javascript:Link.editActivityOccurrence('{0}', '{1}')", activityId, startTime) 
            : string.Format("javascript:Link.editActivity('{0}')", activityId);
        return retVal;
    }
    ...
    private void SetDuration()
    {
        if (_CurrentActivity.Duration < 60)
            lblDurationText.Text = String.Format(GetLocalResourceObject("lblDuration_Minutes.Text").ToString(), _CurrentActivity.Duration);
    ...
    {
        return timeless
            ? value.ToString(CultureInfo.CurrentCulture.DateTimeFormat.ShortDatePattern)
    ...
    private ICollection<string> GetListNonRecurActivityIds()
    {
        if (string.IsNullOrEmpty(NonRecurActivityIds.Value))
            return new string[0];
        return NonRecurActivityIds.Value.Split(',');
    }
    ...
    protected void grdActivityReminders_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        var recurring = item["Recurring"].ToString();
        ...
        checkBox.InputAttributes["data-recurring"] = recurring;
    ...
    protected class StringResources
    {
        ...
        public string NotAllowedOnRecurring;
    ...
    protected void Page_Init(object sender, EventArgs e)
    {
        SR = new StringResources
        {
            ...
            NotAllowedOnRecurring = Res("NotAllowedOnRecurring"),
