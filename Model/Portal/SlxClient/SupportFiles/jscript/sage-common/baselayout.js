            ...
            if (addit) {
                if ((navItems[i].isspacer == "True") || (navItems[i].text == '-')) {
                    ...
                    if (navItems[i].submenu.length > 0) {
                        ...
                        menu.add({
                            text: navItems[i].text, menu: newsubmenu,
                            cls: "x-btn-text-icon", iconCls: 'no-icon', hideOnClick: false
                        }); //id: navItems[i].id, 
                    } else {
...
function populateGroupTabs() {
    ...
    if (typeof GroupsMenu !== "undefined") {
        GroupsMenu.destroy();
    }
    ...
    if (groupMenuList.groups) {
        ...
        for (var i = 0; i < groupMenuList.groups.length; i++) {
            var txt = groupMenuList.groups[i].text;  
            if (currentGroupId == groupMenuList.groups[i].id) {
                activeIdx = i;
            }
            GroupsMenu.add({ text: txt, handler: makeLink(groupMenuList.groups[i].href) });
            tabItems.push({ title: txt, html: '', url: groupMenuList.groups[i].href, id: groupMenuList.groups[i].id });
        }
        ...
        var tabBarWidth = tabCount == 0 ? 5000 : tabCount * 600;
        ...
        if (groupMenuList.groups.length > 0) {
            ...
            document.getElementById('lookupBtn').innerHTML = '';
            ...
            if ((typeof Sage.SalesLogix != 'undefined') && (document.location.href.toLowerCase().indexOf('showlookuponload=true') > -1)  && !(Sage.Lookupdisplayed)) {
                showMainLookup();
            }
        ...
        if (typeof MasterPageLinks != "undefined") {
            document.getElementById('GroupMenuSpace').innerHTML = '';
            var groupTb = new Ext.Toolbar({
                id: "GroupsMenuToolBar",
                cls: "nobackground",
                renderTo: "GroupMenuSpace",
                stateful: false
            });
...
$(document).ready(function() {
    ...
    if (gMgrSvc) gMgrSvc.addListener(Sage.GroupManagerService.CURRENT_GROUP_CHANGED, handleGroupChange);
});
...
function checkforShowLookup() {
    ...
    if ((typeof listPanel != 'undefined') && (document.location.href.toLowerCase().indexOf('showlookuponload=true') > -1)  && !(Sage.Lookupdisplayed)) {
        showMainLookup();
    }
}
...
function showContextMenu(x, y, contextmenu) {
    ...
    var dispose = function() { leftNavContextMenu.destroy(); };
    if (Ext.isIE7) {
        Ext.getBody().on("click", function() { window.setTimeout(dispose, 200); });
        Ext.getBody().on("contextmenu", function() { window.setTimeout(dispose, 200); });
    } else {
        Ext.getBody().on("click", dispose);
        Ext.getBody().on("contextmenu", dispose);
    }
    if (typeof idMenuItems != "undefined") {
...
function assertMenuHeight(m) {
    ...
    var maxWidth = Ext.getBody().getWidth(); //ie8 calcs the scrollbar after... 
    if ((Ext.isIE8) && ((box.x + m.el.getComputedWidth() + 1) > maxWidth)) {
        m.el.setLeft(maxWidth - m.el.getComputedWidth() - 1);
    }
