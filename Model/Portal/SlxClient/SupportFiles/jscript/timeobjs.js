    ...
    if (dayEnd == dayStart) {
        ...
        if ((currTimeMin - 0) <= (dayStart - 0)) {
            ...
            if (addMin < workDayMin) {
                ...
                nonworktimeperiods = parseInt(addMin / workDayMin, 10);
                nonworkmin = nonworktimeperiods * (1440 - workDayMin);
            ...
            /* Scenario 2B- current time is during the work day.                      */ if ((currTimeMin - 0) <= (dayEnd - 0)) {
            /* Will we end today?   */
            if (addMin < (dayEnd - currTimeMin)) {
                /* yes, we only have today to worry about                           */
            } else {
                /* no, we need to account for office closed time.                   */
                nonworktimeperiods = 1 + (parseInt((addMin - (dayEnd - currTimeMin)) / workDayMin, 10));
            ...
            this.setMinutes(this.getMinutes() + (addMin - 0));
                ...
                /* Scenario 2C- current time is after the work day.                     */ if ((currTimeMin - 0) > (dayEnd - 0)) {
        ...
        /*  open past midnight. Not common, but it is possible.                 */ if ((dayStart - 0) > (dayEnd - 0)) {
        /* How many minutes in a work day?                                    */
        workDayMin = 1440 - (dayStart - dayEnd);
        ...
        if ((currTimeMin - 0) < (dayEnd - 0)) {
            ...
            /* Scenario 3B - current time is before we open this evening          */ if ((currTimeMin - 0) < (dayStart - 0)) {
            ...
            this.setMinutes((dayStart - 0) + (addMin - 0));
                ...
                /* Scenario 3C - current time is after we open in the evening         */ if ((currTimeMin - 0) >= (dayStart - 0)) {
            ...
            if ((addMin - 0) < (worktimelefttoday - 0)) {
                ...
                nonworktimeperiods = 1 + (parseInt((addMin - (workDayMin)) / workDayMin, 10));
                /*     1 = time between today and tomorrow                          */
                /*     addMin = minutes to add from now                             */
...
function Time_setTimeFrame(obj) {
    ...
    var timeFrame = (!obj.options) ? obj : obj.options[obj.selectedIndex].value;
    if (timeFrame == "notime") {
        this.setAllSpan();
    } else {
...
function Time_fmtServerToJS(datestr) {
    ...
    if (dateParts[yy].length == 2) {
        var intYear = Number(dateParts[yy]);
