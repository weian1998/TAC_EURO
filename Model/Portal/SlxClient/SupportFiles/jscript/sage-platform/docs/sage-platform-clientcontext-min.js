...
return-1;};_indexOfNoCase=function(key){for(i=0;i<_items.length;i++){if(_items[i].itemKey.toUpperCase()==key.toUpperCase()){return i;}}
...
else{_throwKeyNotFound(key);}},getValueEx:function(key,nocase){var index;if(nocase)
index=_indexOfNoCase(key);else
index=_indexOf(key);if(index!==-1){return _items[index].itemVal;}
else{_throwKeyNotFound(key);}},clear:function(){_items=[];this.save();},containsKey:function(key){return(_indexOf(key)!==-1);},containsKeyEx:function(key,nocase){if(nocase)
return(_indexOfNoCase(key)!==-1);else
return(_indexOf(key)!==-1);},getCount:function(){return _items.length;},hasKeys:function(){return _items.length===0;},getKeys:function(){var keyRes=[];for(i=0;i<_items.length;i++){keyRes.push(_items[i].itemKey);}
