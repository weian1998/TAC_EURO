...
bindingManager.rollbackCurrentTransaction();self._dialogInfo={};var value=Sys.Serialization.JavaScriptSerializer.serialize(evt);$("#"+self._stateClientId).val(value);Sage.DialogWorkspace.requestMask();__doPostBack(self._stateClientId,'');return false;});dialog.on('hide',function(panel){self.fireEvent('close',this);});dialog.on('resize',function(window,width,height){if(Ext.isString(dialog.layout)){dialog.layout=new Ext.Container.LAYOUTS[dialog.layout.toLowerCase()](dialog.layoutConfig||{});dialog.setLayout(dialog.layout);}
dialog.doLayout();});this._dialogPanel=content;this._dialog=dialog;};Sage.DialogWorkspace.prototype.fitToViewport=function(size){var win={width:$(window).width(),height:$(window).height()};var el={width:$(".dialog-workspace-content").width(),height:$(".dialog-workspace-content").height()};var frame={width:0,height:0};if(el.width<=0||el.height<=0)
el={width:0,height:0};var out={width:(size.width+frame.width),height:(size.height+frame.height),left:false,top:false,scroll:false};if(out.height>win.height){out.height=win.height;out.top=0;out.scroll=true;}
if(out.width>win.width){out.width=win.width;out.left=0;}
