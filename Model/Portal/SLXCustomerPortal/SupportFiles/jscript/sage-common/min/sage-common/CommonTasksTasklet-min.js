...
Sage.namespace('Sage.TaskPane.CommonTasksTasklet');var contextSvc;var context;var strEntityId;var strEntityDescription;var strEntityTableName;var selections;var arraySelections=new Array;var activeGrid;var postUrl="";var totalCount;var sortExpression='';var selectedRecordActionCallback;var selectionInfo;Sage.TaskPane.CommonTasksTasklet=function(options){this._id=options.id;Sage.TaskPane.CommonTasksTasklet.clientID=options.clientId;};Sage.TaskPane.CommonTasksTasklet.prototype.init=function(){var prm=Sys.WebForms.PageRequestManager.getInstance();prm.add_pageLoaded(function(sender,args){var l=Sage.SalesLogix.Controls.ListPanel.find("MainList");if(l){$("#selectionCount").text(l.getTotalSelectionCount());}});};function GetCurrentEntity(){if(Sage.Services.hasService("ClientEntityContext")){contextSvc=Sage.Services.getService("ClientEntityContext");context=contextSvc.getContext();strEntityId=context.EntityId;strEntityType=context.EntityType;strEntityTableName=context.EntityTableName;}}
...
if(firstid!=""){var cec=Sage.Services.getService('ClientEntityContext');if(cec){cec.navigateSLXGroupEntity(firstid,strEntityId);}}}
...
{alert(error)},success:function(status){__doPostBack("MainContent","");}});}}
...
{}}
StoreAllSelectionsOrCancel=function(agree){if(agree=="yes"){var selectionKey="selectAll";document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value=selectionKey;var svc=Sage.Services.getService("SelectionContextService");svc.setSelectionContext(selectionKey,selectionInfo,selectedRecordActionCallback);}
else{document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value="cancel";}}
...
document.cookie="format="+formatType+"; expires=1/01/2020";document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_tskExportToExcel').click();dialog.hide();}},{text:CommonTasksResx.ExportToFile_Cancel,handler:function(){dialog.hide();}}]});dialog.show();}
function EmailSend(){GetCurrentEntity();var url="slxdata.ashx/slx/crm/-/namedqueries?columnaliases=email&format=json&hql=";var sHql=GetURL(strEntityTableName);if(sHql==null){Ext.Msg.show({title:"Sage SalesLogix",msg:MasterPageLinks.AdHocDialog_NoEmail,buttons:Ext.Msg.OK,icon:Ext.MessageBox.ERROR});return;}
url=url+sHql;$.ajax({url:url,dataType:'json',success:Send,error:function(error){alert(error.StatusText);}});}
function Send(email){if(email.count>0){var sEmail=email.items[0].email;if(sEmail){sEmail="mailto:"+sEmail;document.location.href=sEmail;return;}}
Ext.Msg.show({title:"Sage SalesLogix",msg:MasterPageLinks.AdHocDialog_NoEmail,buttons:Ext.Msg.OK,icon:Ext.MessageBox.ERROR});}
function GetURL(entity){var url=null;switch(entity){case'CONTACT':url="select con.Email from Contact con where con.id like '"+strEntityId+"'";break;case'ACCOUNT':url="select con.Email from Contact as con where con.Account.id like '"+strEntityId+"' and con.IsPrimary like 'T'";break;case'OPPORTUNITY':url="select con.Email from Opportunity as opp left join opp.Contacts as oppCon left join oppCon.Contact as con where opp.id like '"+strEntityId+"' and oppCon.IsPrimary like 'T'";break;case'LEAD':url="select le.Email from Lead le where le.id like '"+strEntityId+"'";break;case'CAMPAIGN':url="select ufo.Email from Campaign as cam left join cam.AccountManager as usr left join usr.UserInfo as ufo where cam.id like '"+strEntityId+"'";break;case'TICKET':url="select ufo.Email from User as usr left join usr.UserInfo as ufo where usr.DefaultOwner in  (select own.id from Ticket as tic left join tic.AssignedTo as own where tic.id like '"+strEntityId+"')";break;case'DEFECT':url="select ufo.Email from User as usr left join usr.UserInfo as ufo where usr.DefaultOwner in (select own.id from Defect as def left join def.AssignedTo as own where def.id like '"+strEntityId+"')";break;case'CONTRACT':url="select con.Email from Contract as crt left join crt.Contact as con where crt.id like '"+strEntityId+"'";break;case'SALESORDER':url="select so.ShippingContact.Email from SalesOrder as so where so.Id eq '"+strEntityId+"'";break;}
return url;}
...
else{document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value=selectionInfo.key;var svc=Sage.Services.getService("SelectionContextService");svc.setSelectionContext(selectionInfo.key,selectionInfo,selectedRecordActionCallback);}}
function CopyUser(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.copyUser(selectionKey);}
function CopyUserProfile(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.copyUserProfile(selectionKey);}
function DeleteUsers(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.deleteUsers(selectionKey);}
function AddToTeam(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.addToTeam(selectionKey);}
function RemoveFromTeam(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.removeFromTeam(selectionKey);}
function RemoveFromAllTeams(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.removeFromAllTeams(selectionKey);}
function AssignRole(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.assignRole(selectionKey);}
function ReplaceTeamMember(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.replaceTeamMember(selectionKey);}
function DeleteTeam(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.deleteTeam(selectionKey);}
function DeleteDepartment(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.deleteDepartment(selectionKey);}
function CopyTeam(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
Link.copyTeam(selectionKey);}
function CopyDepartment(){var selectionKey=document.getElementById(Sage.TaskPane.CommonTasksTasklet.clientID+'_hfSelections').value
