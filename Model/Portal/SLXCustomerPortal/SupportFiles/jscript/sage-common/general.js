...
function hideRequestIndicator(sender, args) {
    ...
    if (args.get_error != undefined && args.get_error() != undefined) {
        if (args.get_response() != null && args.get_response().get_responseAvailable() && args.get_response().get_statusCode() == '0') {
            args.set_errorHandled(true);
            return;
        }

        if (args.get_response() != null && (args.get_response().get_timedOut() || (args.get_response().get_responseAvailable() && args.get_response().get_statusCode() == '200'))) {
        ...
        args.set_errorHandled(true);
        var regx = new RegExp("^.*:");
        errorMessage = errorMessage.replace(regx, "");
...
function DoGeneralSearchOptionsPage_init() {
    if (document.getElementById("FaxProviderOptions")) {
        ...
        if (typeof (FaxProviderControl) != "undefined") {
            ...
            var oService = GetMailMergeService(false);
            if (oService) {
            ...
            var oFaxProviderLabel = $get("FaxProviderLabel");
            if (oFaxProviderLabel) {
                oFaxProviderLabel.style.display = "none";
            }
        }
    }
}
...
function GeneralSearchOptionsPage_init() {
    window.setTimeout('DoGeneralSearchOptionsPage_init()', 500);
}
        ...
        for(var i = 0; i < navItems.length; i++) {
            if ((navItems[i].isspacer == "True") || (navItems[i].text == '-')) {
                ...
                if (navItems[i].submenu.length > 0) {
                    var newsubmenu = new Ext.menu.Menu( {id: navItems[i].id+'submenu' } );
...
function showMainLookupFor(family, page) {
    if (typeof family === "undefined" || typeof page === "undefined") {
        Ext.Msg.show({ title: "Sage SalesLogix", msg: "The parameters are incorrectly defined for showMainLookupFor().", buttons: Ext.Msg.OK, icon: Ext.MessageBox.ERROR });
        return;
    }
    Sage.Lookupdisplayed = true;
    ...
    var bFamilyIsActive = false;
    var bIsList = false;
    var grpInfo = getCurrentGroupInfo();
    if (grpInfo != "") {
        /* Make sure there is a Family; there won't be on a non-entity page where the ClientGroupContext is not initialized (e.g. Welcome page). */
        if (grpInfo.Family != null) {
            if (grpInfo.Family.toUpperCase() == family.toUpperCase()) {
                /* Make sure main page for the entity is being displayed and not a page such as Calendar. */                  
                if ((document.location.href.toUpperCase().indexOf(page.toUpperCase())) != -1) {
                    bIsList = (modeid.toUpperCase() == "LIST");
                    bFamilyIsActive = true;
                }
            }
        }
    }
...
Error.prototype.toMessage = function(xtraMsg, showFullStack) {
    var sError = "";
    if (Ext.isDefined(xtraMsg) && !Ext.isEmpty(xtraMsg) && Ext.isString(xtraMsg)) {
        sError = xtraMsg;
    ...
    if (Ext.isIE) {
        ...
        sError = String.format("{0}{1} Details: name={2}; number={3}.",
        ...
        var sSource = "";
        var sStack = "";
        if (Ext.isDefined(this.lineNumber)) {
            iLineNumber = this.lineNumber;
        }
        if (Ext.isDefined(this.stack)) {
            sStack = this.stack;
            if (Ext.isBoolean(showFullStack) && showFullStack) {
                sSource = sStack;
            }
            else {
                if (!isNaN(iLineNumber)) {
                    var iPos = sStack.indexOf(String.format(".js:{0}", iLineNumber)); //DNL
                    if (iPos != -1) {
        ...
        if (Ext.isEmpty(sSource)) {
            sSource = "unknown";
        }
        if (iLineNumber == -1) {
            iLineNumber = "unknown";
        }
        sError = String.format("{0}{1} Details: name={2}; line={3}; source={4}.",
