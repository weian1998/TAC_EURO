...
PickListComboBox = function(options) {
    ...
    this.pickListName = o.pickListName; 
    
    var m = { text: "text", id: "itemId", code: "code" };       
    this.store = o.store = new Ext.data.Store({
        baseParams: {   
            name: o.pickListName,
            sort: o.sort            
        },
        proxy: new Ext.data.HttpProxy({
            url: String.format(this.pickListRequestUrl, escape(this.pickListName)),
            method: "GET"
        }),
    ...
    o.valueField = "value";
    o.triggerAction = "all";
    o.minChars = (typeof o.minChars === "number") ? o.minChars : 4;
    o.typeAhead = (typeof o.typeAhead === "boolean") ? o.typeAhead : false;
    o.editable = (typeof o.editable === "boolean") ? o.editable : !o.mustExistInList;
    o.sort = (typeof o.sort === "boolean") ? o.sort : true;
...
Ext.reg('picklistcombo', PickListComboBox);

AddressFormPanel = Ext.extend(Ext.form.FormPanel, {
    setLayout: function(layout) {
        AddressFormPanel.superclass.setLayout.call(this, layout);
        
        if (Ext.isIE6)
        {
            layout.elementStyle = "padding-left:0px;";
        }
    }
});
...
function AddressControl(options) {
    ...
    this._displayValueClientId = options.displayValueClientId;
    this._returnValueClientId = options.returnValueClientId;
...
AddressControl.fieldOptionHandlers = {
    "checkbox": function(f, o) {
        if (o.disableOnChecked)
            if (f.getValue() === true)
                f.disable();
            else
                f.enable();
...
AddressControl.prototype.getDialog = function() { return this._dialog; };

AddressControl.prototype.initContext = function() {
    this._context = document.getElementById(this._clientId);
};

AddressControl.prototype.init = function() {
    this.initContext();
};

AddressControl.prototype.close = function() {
    if (this.getDialog())
        this.getDialog().hide();
};

AddressControl.prototype.ensureDialog = function() {
    if (!this.getDialog())
        this.createDialog();
};
...
AddressControl.prototype.createFormItems = function() {
    ...
    for (var i = 0; i < this._fields.length; i++)  
    {
        ...
        this._map[this._fields[i].name] = this._fields[i];
        var f = $.extend(this._fields[i], this._fields[i].pickList, {
            id: this._clientId + "_field_" + this._fields[i].name,
            stateful: false,
...
AddressControl.prototype.processFieldOptions = function(f, o) {
    if (AddressControl.fieldOptionHandlers[f.xtype])
        AddressControl.fieldOptionHandlers[f.xtype].call(this, f, o);
};

AddressControl.prototype.restoreValues = function() {
    var form = this._form.getForm();
    
    form.setValues(this.getBoundValues()); 
    
    for (var name in this._map)
    {       
        var f = form.findField(name);        
        var o = this._map[name].options;
        if (f && o)
            this.processFieldOptions(f, o);
    }
};

AddressControl.prototype.getBoundValues = function() {
    var values = {};
    for (var name in this._map)
    {
        var field = this._map[name];        
        if (field)
        {
            if (field.pickList) 
            {
                var textEl = document.getElementById(field.pickList.formTextClientId);
                var valueEl = document.getElementById(field.pickList.formValueClientId);
                if (textEl && valueEl)
                    values[name] = {text: $(textEl).val(), value: $(valueEl).val()};
            }
            else
            {
                var el = document.getElementById(field.formClientId);
                if (el)
                    if ($(el).is(":checkbox"))
...
AddressControl.prototype.setBoundValues = function(values) {
    for (var name in values)
    {
        ...
        if (field)
        {
            ...
            else
            {
                ...
                if (el)
                    if ($(el).is(":checkbox"))                
                        el.checked = (value === true || value === "true" || value === "on" || value === 1) ? true : false;                
                    else
                        $(el).val(value);                
            }    
        }
...
AddressControl.prototype.setReturnValue = function(values) {
    var variables = {};
    for (var key in values)
        variables[key] = (typeof values[key] === "object") ? values[key].text : values[key];
            
    var template = new Ext.XTemplate(this._templates.returnValue);
    $("#" + this._returnValueClientId).val(template.apply(variables));
};
...
AddressControl.prototype.setDisplayValue = function(values, complete) {
    ...
    $.get("SLXAddressHandler.aspx", parameters, function(data, status) {         
        $("#" + self._displayValueClientId).val(data);
        if (complete) 
            complete.call();     
    });
};

AddressControl.prototype.getValues = function() {
    var form = this._form.getForm();
    var values = {};
    for (var name in this._map)
    {
...
AddressControl.prototype.saveValues = function() {
    var self = this;
    var values = this.getValues();
     
    this.setBoundValues(values);
    this.setReturnValue(values);
    var mgr = Sage.Services.getService("ClientBindingManagerService");
    if (mgr) {
        mgr.markDirty();
    }    
    this.setDisplayValue(values, function() {
...
AddressControl.prototype.showMap = function() {
    ...
    for (var key in parameters)
        queryParams.push(key + "=" + encodeURIComponent(parameters[key]));
...
function Address_FormatAddress(ID)
{
    var elem = document.getElementById(this.displayID);
    var test = elem.value;
    var vURL = "SLXAddressHandler.aspx?addr1="+this.desc1value+
    "&addr2="+this.desc2value+"&addr3="+this.desc3value
    +"&city="+this.cityvalue+"&st="+this.statevalue
    +"&zip="+this.zipvalue+"&cntry="+this.countryvalue;
...
function Address_ButtonClick()
{
	...
	addElems = new Array();
	addElems[0] = "streetaddress=";
	addElems[1] = "city=";
	addElems[2] = "state=";
	addElems[3] = "zip=";
	addElems[4] = "country=";

    addElems[0] += ((this.desc1value == "") ? "" : this.desc1value);
    addElems[1] += ((this.cityvalue == "") ? "" : this.cityvalue);
    addElems[2] += ((this.statevalue == "") ? "" : this.statevalue);
    addElems[3] += ((this.zipvalue == "") ? "" : this.zipvalue);
    addElems[4] += ((this.countryvalue == "") ? "" : this.countryvalue);
	...
	for (i = 0; i < addElems.length; i++) 
	{
		...
		if (i == 0) 
		{
			tempStr = "http://www.mapquest.com/cgi-bin/ia_free?height=300&width=500&" + addElems[i];
		} else {
            if (addElems[i].indexOf("=") != "") 
		    {
			    tempStr += ("&" + addElems[i]);
			} else {
			    tempStr += ("+" + addElems[i]);
	...
	var find = " ";
	var rep = "+";
	p1 = tempStr.indexOf(find);
	while (p1 != -1) 
	{
		preS = tempStr.substring(0, p1);
		posS = tempStr.substring(p1 + 1, tempStr.length);
		tempStr = preS + rep + posS;
		p1 = tempStr.indexOf(find);
	}
	tempStr += "&level=9&iconid=11";
	OpenMap(tempStr);
}
...
function Address (clientId, desc1, desc2, desc3, city, state, zip, country, county, salutation, description, primary, mailing, autoPostBack)
{
    ...
    this.dlgDivID = clientId + "_outerDiv";
    this.returnValueId = clientId + "_resultText";;
    this.desc1value = desc1;
    this.desc2value = desc2;
    this.desc3value = desc3;
    this.cityvalue = city;
...
function Address_Show()
{
        ...
        {
            var dlgDiv = document.getElementById(this.dlgDivID);
            dlgDiv.style.display = "block";
            this.panel = new YAHOO.widget.Panel(this.dlgDivID, { visible:false, width:"300px", fixedcenter:false, constraintoviewport:true, x:250, y:200, underlay:"shadow", draggable:true });
            this.panel.render();
        }
        this.panel.show();
        document.getElementById(this.clientId + "_Addr1").focus();
}

function Address_Cancel()
{
    this.panel.hide();
    document.getElementById(this.clientId + "_Addr1").value = this.desc1value;
    document.getElementById(this.clientId + "_Addr2").value = this.desc2value;
    document.getElementById(this.clientId + "_Addr3").value = this.desc3value
    document.getElementById(this.clientId + "_City_Text").value = this.cityvalue;
    document.getElementById(this.clientId + "_State_Text").value = this.statevalue;
    document.getElementById(this.clientId + "_Zip").value = this.zipvalue;
    document.getElementById(this.clientId + "_Country_Text").value = this.countryvalue;
    document.getElementById(this.clientId + "_Attn").value = this.salutationvalue;
...
{
    this.panel.hide();
    this.desc1value = document.getElementById(this.clientId + "_Addr1").value;
    this.desc2value = document.getElementById(this.clientId + "_Addr2").value;
    this.desc3value = document.getElementById(this.clientId + "_Addr3").value;
    this.cityvalue = document.getElementById(this.clientId + "_City_Text").value;
    this.statevalue = document.getElementById(this.clientId + "_State_Text").value;
    this.zipvalue = document.getElementById(this.clientId + "_Zip").value;
    this.countryvalue = document.getElementById(this.clientId + "_Country_Text").value;
    this.salutationvalue = document.getElementById(this.clientId + "_Attn").value;
...
Address.prototype.FormatAddress = Address_FormatAddress;
Address.prototype.Show = Address_Show;
...
DependencyLookup.prototype.Show = function () {
    if (this.CanShow()) {
        if (this.panel == null) {
            ...
            this.panel = new YAHOO.widget.Panel(this.PanelId, { visible: false, width: this.Size, fixedcenter: true, constraintoviewport: true, underlay: "shadow", draggable: true, iframe: true, modal: false });
            this.panel.render();
...
Sage.SalesLogix.Controls.BufferedGridView = Ext.extend(Ext.ux.grid.BufferedGridView, {
    ...
    adjustVisibleRows: function() {
        ...
        var vw = (this.scrollOffset) ? size.width - this.scrollOffset : size.width;
        ...
        if (this.visibleRows == visibleRows - this.rowClipped) {
            return;
        }
...
Sage.SalesLogix.Controls.BufferedStore = Ext.extend(Ext.ux.grid.BufferedStore, {
    //Override the Load Records Method because o.toalRecords = null and throws error;
...
Sage.SalesLogix.Controls.ListPanel = Ext.extend(Ext.Panel, {
    ...
    initComponent: function() {
        ...
        Ext.applyIf(this, {
            ...
            bufferResize: true,
            cls: "list-panel",
            disableDetailView: (this.managers.detail ? false : true) 
        });
...
Sage.SalesLogix.Controls.ListPanel.prototype.initToolbar = function() {
    ...
    if (this.detailView)
    {
        ...
        separator.render = function(td) {
            this.td = td;
            if (this.el) td.appendChild(this.el);
            if (this.hidden)
...
if (this.views.timeline)
    items.push({
        slxUpdate: function(l) { this.suspendEvents(); this.toggle(l.activeView == "timeline"); this.resumeEvents(); },
...
if (this.helpUrl)
    items.push({
        ...
        tooltip: Sage.SalesLogix.Controls.Resources.ListPanel.Help,
        text: false,
...
Sage.SalesLogix.Controls.ListPanel.prototype.updateToolbarItems = function() {
    ...
    for (var i = 0; i < items.length; i++)
        if (items[i].slxUpdate)
        items[i].slxUpdate.call(items[i], this);
};
...
Sage.SalesLogix.Controls.ListPanel.prototype.createList = function() {
    ...
    var view = new Sage.SalesLogix.Controls.BufferedGridView({
        ...
        nearLimit: 25,
        forceFit: (this.getViewOptions("list").fitToContainer === false) ? false : true, 
        autoFill: true,
        loadMask: {
    ...
    var selectionModel = new Sage.SalesLogix.Controls.BufferedRowSelectionModel({
        singleSelect: (this.getViewOptions("list").singleSelect === true) ? true : false 
    });
    ...
    grid.on("show", function() {
        if (grid.rendered)
            ...
            grid.on("render", function() { handleShow.call(this);  }, this, { single: true });
    }, this);
...
Sage.SalesLogix.Controls.ListPanel.prototype.createSummary = function() {
    ...
    var selectionModel = new Ext.ux.grid.BufferedRowSelectionModel({
        singleSelect: (this.getViewOptions("summary").singleSelect === false) ? false : true 
    });
...
Sage.SalesLogix.Controls.ListPanel.prototype.getSelectionInfo = function() {
    ...
    var sortDirection = this.views.list.view.ds.sortInfo ? this.views.list.view.ds.sortInfo.direction : '';
    var sortField = this.views.list.view.ds.sortInfo ? this.views.list.view.ds.sortInfo.field : '';
...
Sage.SalesLogix.Controls.ListPanelToolbar = Ext.extend(Ext.Toolbar, {
    ...
    bind: function() {
        var parent = this.findParentBy(function(c) { return true; }, this);
...
Sage.SalesLogix.Controls.GroupSummaryMetaConverter = Ext.extend(Sage.SalesLogix.Controls.MetaConverter, {
    ...
    toColumnModel: function() {
        ...
        if (svc) 
        {
            ...
            var item = {
                ...
                align: 'left',
                sortable: false,
                dataIndex: svc.getContext().CurrentTableKeyField,
    ...
    toReaderConfig: function() {
        ...
        var svc = Sage.Services.getService("ClientGroupContext");
        if (svc)
            id = svc.getContext().CurrentTableKeyField;
...
Sage.UserTypeToDisplayName = function(value) {
    var userTypes = { 'W': Sage.SalesLogix.Controls.Resources.ListPanel.Administrator,
        'P': Sage.SalesLogix.Controls.Resources.ListPanel.Template,
        'M': Sage.SalesLogix.Controls.Resources.ListPanel.Remote,
        'T': Sage.SalesLogix.Controls.Resources.ListPanel.WebOnly,
        'R': Sage.SalesLogix.Controls.Resources.ListPanel.Retired,
        'C': Sage.SalesLogix.Controls.Resources.ListPanel.Concurrent,
        'V': Sage.SalesLogix.Controls.Resources.ListPanel.WebViewer,
        'N': Sage.SalesLogix.Controls.Resources.ListPanel.Network,
        'A': Sage.SalesLogix.Controls.Resources.ListPanel.AddOnUser
    };
    return (userTypes[value] ? Ext.util.Format.htmlEncode(userTypes[value]) : value);
};

Sage.OwnerTypeToDisplayName = function(value) {
    var ownerTypes = { 'G': Sage.SalesLogix.Controls.Resources.ListPanel.Team,
        'D': Sage.SalesLogix.Controls.Resources.ListPanel.Department,
        'S': Sage.SalesLogix.Controls.Resources.ListPanel.System,
        'U': Sage.SalesLogix.Controls.Resources.ListPanel.User
    };
    return (ownerTypes[value] ? Ext.util.Format.htmlEncode(ownerTypes[value]) : value);
};
        ...
        try {
            ...
            for (var i = 0; i < l; i++) {
                ...
                switch (true) {
                    ...
                    case (column.dataPath == "USERSECURITY:TYPE"):
                        column.renderer = this.createUserTypeRenderer(column);
                        break;
    ...
    createEntityLinkRenderer: function (column) {
        ...
        return function (value, meta, record, rowIndex, columnIndex, store) {
            ...
            if ((entity.toUpperCase() == 'HISTORY') || (entity.toUpperCase() == 'ACTIVITY')) {
                ...
                return (value ? String.format(fmtstr, id, valuerenderer(value, meta, record, rowIndex, columnIndex, store), entity) : "");
            } else {
                var groupid = '';
...
function LookupControl(options) {
    ...
    this._id = options.id;
    this._clientId = options.clientId;
    this._gridId = options.gridId;
    this._namedQuery = options.namedQuery;
    this._seedProperty = options.seedProperty;
    this._seedValue = options.seedValue;
    this._currentSeedValue = options.seedValue;
    this._overrideSeedOnSearch = options.overrideSeedOnSearch;
    this._entity = options.entity;
    this._entityDisplayName = options.entityDisplayName;
    this._entityDisplayProperty = options.entityDisplayProperty;
    this._linkUrl = options.linkUrl;
    this._resultId = options.resultId;
    this._resultTextId = options.resultTextId;
    this._resultHyperTextId = options.resultHyperTextId;
    this._properties = options.properties;
    this._context = null;
    this._dialog = null;
    this._dialogHeight = options.dialogHeight;
    this._dialogWidth = options.dialogWidth;
    this._dialogTitle = options.dialogTitle || String.format(LookupControlResources.LookupControl_Header, this._entityDisplayName);
    this._autoPostBack = options.autoPostBack;
    this._initializeLookup = options.initializeLookup;
    this._initialized = false;
    this._helpLink = options.helpLink;
    this._returnPrimaryKey = options.returnPrimaryKey;
    this._returnProperty = 'id';
   
    this._templates = {
        lookup: new Ext.XTemplate(
            '<div class="lookup-query">',
                '<span class="lookup-label">{lookupLabel}</span>',
                '<div class="lookup-properties">',
                    '<select id="{lookupPropertiesId}" tabindex="1">',                    
                    '</select>',
                '</div>',
                '<div class="lookup-operators">',
                    '<select id="{lookupOperatorsId}" tabindex="2">',                    
                    '</select>',
                '</div>',
                '<div class="lookup-values">',
                '<select id="{lookupValuesSelectId}" style="display:none;" tabindex="3"></select><input id="{lookupValuesInputId}" style="display:none;" type="text" tabindex="3" />',
                '</div>',
                '<div class="lookup-search"></div>', 
    ...
    {
        this._propertyPairs[this._properties[i].value] = this._properties[i].name;
        this._propertyLookup[this._properties[i].value] = this._properties[i];
        if (this._properties[i].useAsResult) this._returnProperty = String(this._properties[i].value).replace(".", "_");
    }

    LookupControl.__instances[this._clientId] = this;
    LookupControl.__initRequestManagerEvents();
...
Ext.extend(LookupControl, Ext.util.Observable);

LookupControl.DISPLAY_MODE_LIST = 0;
LookupControl.DISPLAY_MODE_DIALOG = 1;
LookupControl.DISPLAY_MODE_LINK = 2;
LookupControl.DISPLAY_MODE_TEXT = 3;
LookupControl.DISPLAY_MODE_BUTTON = 4;
...
LookupControl.__defaultOperators = [    
    {name: "LookupControl_Operator_Equal", value: "eq"},
    {name: "LookupControl_Operator_NotEqual", value: "ne"},
    {name: "LookupControl_Operator_GT", value: "gt"},
    {name: "LookupControl_Operator_GE", value: "ge"},
...
LookupControl.__transforms = {
    standard: {
        sw: function(e,p,t,v) { return String.format("{0}.{1} like {2}", e, p, this.escape(t,(v+"*"),true)); },
        ct: function(e,p,t,v) { return String.format("{0}.{1} like {2}", e, p, this.escape(t,("*"+v+"*"),true)); },
        eq: function(e,p,t,v) { return String.format("{0}.{1} eq {2}", e, p, this.escape(t,v,true)); },
    ...
    upper: {
        ...
        ct: function(e,p,t,v) { return String.format("upper({0}.{1}) like upper({2})", e, p, this.escape(t,("*"+v+"*"),true)); },
        eq: function(e,p,t,v) { return String.format("upper({0}.{1}) eq upper({2})", e, p, this.escape(t,v,true)); },
...
LookupControl.__requestManagerEventsInitialized = false;
LookupControl.__initRequestManagerEvents = function() {
    if (LookupControl.__requestManagerEventsInitialized)
        return;        
        ...
        {            
            for (var i = 0; i < panels.length; i++)
            {
                for (var id in LookupControl.__instances)
                    ...
                    {
                        var instance = LookupControl.__instances[id];                        
    ...
    LookupControl.__requestManagerEventsInitialized = true; 
};

LookupControl.createOrRefresh = function(id, options) {
    var instance = id;
    if (typeof id === "string")
        instance = window[id];
    ...
    {
        instance = window[id] = new LookupControl(options);
        instance.init();
    }
};

LookupControl.prototype.initContext = function() {
    if (document.getElementById)
        this._context = document.getElementById(this._clientId);
};

LookupControl.prototype.isInitialized = function() { return this._initialized; };
LookupControl.prototype.init = function() {
    this._initialized = true;
    this.initContext();
};

LookupControl.prototype.refresh = function(options) {
...
LookupControl.prototype.getTransformFor = function(op)
{
    var store = this._options.forceUpper ? LookupControl.__transforms.upper : LookupControl.__transforms.standard;
    if (store && store[op])
        return store[op];
    else
        return false;       
...
LookupControl.prototype.escape = function(type, value, quote) {    
    switch (type) 
    {
...
LookupControl.prototype.enable = function() {
    var result = $("#" + this._resultId);
    if (result.length > 0)
        result.get(0).disabled = true;
        
    var text = $("#" + this._resultTextId);
    if (text.length > 0)
        text.get(0).disabled = true;
};

LookupControl.prototype.disable = function() {
    var result = $("#" + this._resultId);
    if (result.length > 0)
        result.get(0).disabled = false;
        
    var text = $("#" + this._resultTextId);
    if (text.length > 0)
        text.get(0).disabled = false;
};

LookupControl.prototype.close = function() {
    if (this._dialog)
        this._dialog.hide();
};

LookupControl.prototype.getResult = function() {
    return $("#" + this._resultId).val();
};

LookupControl.prototype.setResult = function(value) {    
    if (typeof value === 'object')
    {
        this.setResultText(
            this._returnPrimaryKey
                ? value[this._entityDisplayProperty] || value['id']
                : value[this._entityDisplayProperty] || value[this._returnProperty]
        );
        this.setResultValue(    
            this._returnPrimaryKey
                ? value['id']
                : value[this._returnProperty]
        );        
    }
    else
...
LookupControl.prototype.getResultValue = function() {
    return $("#" + this._resultId).val();
};

LookupControl.prototype.setResultValue = function(value) {
    $("#" + this._resultId).val(value);
};

LookupControl.prototype.getResultText = function() {
    return $("#" + this._resultTextId).val();
};

LookupControl.prototype.setResultText = function(value) {
    $("#" + this._resultTextId).val(value);
};

LookupControl.prototype.setResultHyperText = function(value) {
    $("#" + this._resultHyperTextId).text(value);
};

LookupControl.prototype.doPostBack = function() {
    __doPostBack(this._clientId, '');
};

LookupControl.prototype.goToResult = function() {
    var result = this.getResult();    
    if (result) {
        var mgr = Sage.Services.getService("ClientBindingManagerService");
        if (mgr) {
            if (mgr.canChangeEntityContext()) { 
                window.location = this._linkUrl + result;
            }
        } else {
            window.location = this._linkUrl + result;
        }
...
LookupControl.prototype.getGrid = function() {
    return window[this._gridId];
};

LookupControl.prototype.invokeChangeEvent = function(el) {
    if (document.createEvent)
    {
        //FF
...
LookupControl.prototype.getSeedQueryExpression = function(seedValue) {
    if (this._seedProperty)
    {
        var property = this._seedProperty;
        var operator = "sw";
        var value = seedValue || this._currentSeedValue;
        
        var transform = this.getTransformFor(operator);
        if (transform)      
            if (value)
...
LookupControl.prototype.setGridParameters = function(seedValue, reload) {
    var seedQuery = this.getSeedQueryExpression(seedValue);
   
    if (seedQuery)
        this.getGrid().getConnections()["data"].parameters["where"] = seedQuery; 
    else
        if (!this._initializeLookup && !this.hasDialog()) 
            this.getGrid().getConnections()["data"].parameters["where"] = "0 eq 1";
    
    if (reload)
        this.getGrid().clearMetaData();                
   
     if (reload && this.getGrid().getNativeGrid())
        this.getGrid().getNativeGrid().getStore().reload();
};

LookupControl.prototype.initGrid = function(seedValue, reload) {
    if (this.getGrid() && this.getGrid().hasStaticLayout()) {
        var isGridInitialized = (this.getGrid().getNativeGrid()) ? true : false;
        var reload = (!isGridInitialized) ? false : reload;

        this.setGridParameters(seedValue, reload);
        this.getGrid().clearMetaData();
    ...
    this.getGrid().getNativeGrid().getTopToolbar().hide();
};

LookupControl.prototype.wireUpGridEvents = function() {        
    var self = this;   
    this.getGrid().getNativeGrid().on('rowdblclick', function(nativeGrid, rowIndex, evt) {
            
            //var selected = nativeGrid.getStore().getAt(rowIndex);
            
            var view = nativeGrid.getView();
            var store = nativeGrid.getStore();
            
            var selected = store.getAt(rowIndex);     
            if (!selected)
                selected = store.data.items[ rowIndex - view.bufferRange[0] ];
...
LookupControl.prototype.checkPostBack = function() {
    if (this._autoPostBack)
        __doPostBack(this._clientId, '');
};

LookupControl.prototype.clearResult = function() {
    this.setResult("");
    this.setResultText("");
    this.setResultHyperText("");
    this.invokeChangeEvent($("#" + this._resultTextId).get(0));
...
LookupControl.prototype.getDialog = function() { return this._dialog; };
LookupControl.prototype.ensureDialog = function() {
    if (!this.getDialog())
        this.createDialog();
};

LookupControl.prototype.show = function(alternateSeedValue) {     
    var reload = false;   
    if (alternateSeedValue && alternateSeedValue != this._currentSeedValue)
    {
        this._currentSeedValue = alternateSeedValue;
        reload = true;
    }       
       
    this.initGrid(alternateSeedValue, reload);       
    this.ensureDialog();       
    this.getDialog().doLayout();
    this.getDialog().show();    
    this.getDialog().center();
    
    this.getGrid().getNativeGrid().getStore().load(); 
...
{
    this.ClientId = clientId;
    this.nameDivId = clientId + "_outerDiv";
    this.displayID = clientId + "_displayText";
    this.returnValueId = clientId + "_nameResult";
    this.prefix = prefix;
    this.first = first;
    this.middle = middle;
    this.last = last;
...
{
    this.panel.hide();
    document.getElementById(this.ClientId + "_prefix_Text").value = this.prefix;
    document.getElementById(this.ClientId + "_first").value = this.first;
    document.getElementById(this.ClientId + "_middle").value = this.middle;
    document.getElementById(this.ClientId + "_last").value = this.last;
    document.getElementById(this.ClientId + "_suffix_Text").value = this.suffix;
}

function NamePanel_Ok()
{
    this.panel.hide();
    this.prefix = document.getElementById(this.ClientId + "_prefix_Text").value;
    this.first = document.getElementById(this.ClientId + "_first").value;
    this.middle = document.getElementById(this.ClientId + "_middle").value;
    this.last = document.getElementById(this.ClientId + "_last").value;
    this.suffix = document.getElementById(this.ClientId + "_suffix_Text").value;
    var returnValue = document.getElementById(this.returnValueId);
    returnValue.value = this.prefix + "|" + this.first + "|" + this.middle + "|" + this.last + "|" + this.suffix;
    this.FormatName();
}
...
function NamePanel_Trim(stringToTrim) {
...
function NamePanel_ParseName()
{
    ...
    var longest = 0;
    var elem = document.getElementById(this.displayID);
    var parseText = elem.value;
    var prefixes = document.getElementById(this.ClientId + "_prefix_Items");
    var suffixes = document.getElementById(this.ClientId + "_suffix_Items");
    var lastNamePrefixes = document.getElementById(this.ClientId + "_lastNamePrefixes");
    var testValue = "";
    //Reset all values
    this.prefix = "";
    this.first = "";
    ...
    //Start with prefixes
    if (prefixes.options.length == 0)
    {
        eval(this.ClientId + "_prefix_obj.GetList();");
    }
    for (var i = 0; i < prefixes.options.length; i++)
    {
        var testPrefix = prefixes.options[i].text.toUpperCase() + " ";
        testValue = parseText.substr(0, testPrefix.length).toUpperCase();
        if (testValue.localeCompare(testPrefix) == 0)
        {
            if (prefixes.options[i].text.length > longest)
            {
                this.prefix = this.Trim(prefixes.options[i].text);
                document.getElementById(this.ClientId + "_prefix_Text").value = this.prefix;
                longest = this.prefix.length;
                break;
            }
        }
        else
        {
            testPrefix = testPrefix.replace(/\./g, "");
            testValue = parseText.substr(0, testPrefix.length).toUpperCase();
            if (testValue.localeCompare(testPrefix) == 0)
            {
                if (prefixes.options[i].text.length > longest)
    ...
    longest = 0;
    lastSpaceIdx = parseText.lastIndexOf(" ");
    testValue = parseText.substr(lastSpaceIdx +1).replace(/\./g, "");
    testValue = testValue.toUpperCase();
    if (suffixes.options.length == 0)
    {
        eval(this.ClientId + "_suffix_obj.GetList();");
    }
    for (var i = 0; i < suffixes.options.length; i++)
    {
        var suffixWithoutDots = suffixes.options[i].text.replace(/\./g, "");
        suffixWithoutDots = suffixWithoutDots.toUpperCase();
        if (parseText.length > suffixes.options[i].text.length +1)
        {
            if (testValue.localeCompare(suffixWithoutDots) == 0)
            {
                if (suffixWithoutDots.length > 0)
                {
                    this.suffix = this.Trim(suffixes.options[i].text);
                    document.getElementById(this.ClientId + "_suffix_Text").value = this.suffix;
                    longest = this.suffix.length;
                }
        ...
        {
            testValue = testValue.substr(testValue.length - suffixWithoutDots.length, suffixWithoutDots.length +1);
            if (testValue.localeCompare(" " + suffixWithoutDots) == 0)
            {
                if (suffixWithoutDots.length > 0)
                {
                    this.suffix = this.Trim(suffixes.options[i].text);
                    document.getElementById(this.ClientId + "_suffix_Text").value = this.suffix;
                    longest = suffixWithoutDots.length;
                    break;
                }
            }
        }
    }
    //alert(parseText + " " + longest);
    if (longest > 0)
    {
        longest++;
        parseText = parseText.substr(0, parseText.length - longest);
    }
    
    //Set Last Name
    lastSpaceIdx = parseText.lastIndexOf(" ");
    //Trim spaces from the end.
    while ((lastSpaceIdx == parseText.length -1) && (lastSpaceIdx > 0))
    {
        parseText = parseText.substr(0, lastSpaceIdx);
        lastSpaceIdx = parseText.lastIndexOf(" ");
    }
    if (lastSpaceIdx > 0)
    {
        this.last = parseText.substr(lastSpaceIdx +1);
        document.getElementById(this.ClientId + "_last").value = this.last;
        parseText = parseText.substr(0, lastSpaceIdx);
    }
    else
    {
        this.last = this.Trim(parseText);
        document.getElementById(this.ClientId + "_last").value = this.last;
        parseText = "";
    }
    
    //Set Middle/Two Word Last
    lastSpaceIdx = parseText.lastIndexOf(" ");
    //alert(parseText + ": last space index: " + lastSpaceIdx);
    if (lastSpaceIdx > 0)
    {
        testValue = parseText.substr(lastSpaceIdx +1).toUpperCase();
        //alert("TestValue: " + testValue);
        for (var i = 0; i < lastNamePrefixes.options.length; i++)
        {
            if (testValue.localeCompare(lastNamePrefixes.options[i].text.toUpperCase()) == 0)
            {
                this.last = lastNamePrefixes.options[i].text + " " + this.last;
                document.getElementById(this.ClientId + "_last").value = this.last;
                parseText = parseText.substr(0, lastSpaceIdx);
                break;
            }
    ...
    //Set First Name
    parseText = this.Trim(parseText);
    lastSpaceIdx = parseText.indexOf(" ");
    //alert(parseText + ": last space index: " + lastSpaceIdx);
    if (lastSpaceIdx > 0)
    {
        this.first = this.Trim(parseText.substr(0, lastSpaceIdx));
        this.middle = this.Trim(parseText.substr(lastSpaceIdx +1));
    }
...
NamePanel.prototype.HandleKeyEvent = NamePanel_HandleKeyEvent;
NamePanel.prototype.NameChanged = NamePanel_NameChanged;
NamePanel.prototype.ParseName = NamePanel_ParseName;
NamePanel.prototype.Trim = NamePanel_Trim;
function OwnerControl(options) {
    ...
    this._helpLink = options.helpLink;
    this._multiSelect = options.multiSelect || false;
    this._types = options.types || "U,T,S";
...
OwnerControl.prototype.createDialog = function() {
    function getChildren(ownertypes) {
        var children = [];
        if (ownertypes.indexOf('U') > -1)
            children.push({
                id: "user",
        ...
        if (ownertypes.indexOf('D') > -1)
            children.push({
                id: "department",
                text: "Department",
                cls: "owner-type-department-container",
                singleClickExpand: true,
                listeners: { "expand": function(node) { self.identifyNode(node); } }
            });
        if (ownertypes.indexOf('S') > -1)
            children.push({
                id: "system",
    ...
    var root = new Ext.tree.AsyncTreeNode({
        ...
        expanded: true,
        listeners: { "expand": function(node) { self.identifyNode(node); } },
        children: getChildren(this._types)
    });
            ...
            buttons: [{
                ...
                handler: function() {
                    if (self._multiSelect) {
                        ...
                        self.close();
                        self.setResult(ids.join(','), texts.join(','));
                        return;
                    }
            ...
            function OwnerPanel_Show() {
                if (this.CanShow()) {
                    if ((this.panel == null) || (this.panel.element.parentNode == null)) {
                        ...
                        lookup.style.display = "block";
                        this.panel = new YAHOO.widget.Panel(this.OwnerDivId, { visible: false, width: "305px", fixedcenter: true, constraintoviewport: true, underlay: "shadow", draggable: true, modal: false });
                        this.panel.render();
                    }
                    this.panel.show();
...
// Control used to select "User" entities for relationships.
function UserControl(options) {
    options = options || {};
    this._dialog = null;
    this._options = options;    
    this._clientId = options.clientId;
    ...
    this._valueClientId = options.valueClientId;
    this._autoPostBack = options.autoPostBack; 
    this._userTypesToShow = options.userTypesToShow;
    this._userTypesToShowOrder = options.userTypesToShowOrder;
    this._identifyNodes = true;   
    this._result = false;
    this._typeAheadText = false;
    this._typeAheadTimeout = false;
    this._helpLink = options.helpLink;
    UserControl.__instances[this._clientId] = this;
    UserControl.__initRequestManagerEvents();
...
Ext.extend(UserControl, Ext.util.Observable);

UserControl.__instances = {};
UserControl.__requestManagerEventsInitialized = false;
UserControl.__initRequestManagerEvents = function() {
    if (UserControl.__requestManagerEventsInitialized)
        return;        
        ...
        {            
            for (var i = 0; i < panels.length; i++)
            {
                for (var id in UserControl.__instances)
                    if (contains(panels[i], document.getElementById(id))) 
                    {
                        var instance = UserControl.__instances[id];   
                        instance.initContext();                                             
                    }                        
            }            
        }
    ...
    UserControl.__requestManagerEventsInitialized = true; 
};
...
UserControl.prototype.initContext = function() {
    this._context = document.getElementById(this._clientId);
};

UserControl.prototype.identifyNode = function(node) {
    if (!this._identifyNodes || node._wasIdentified)
        return;
...
UserControl.prototype.createUserTypeNodes = function() {
    var self = this;
    var children = [];
    for (var i = 0; i < this._userTypesToShowOrder.length; i++)
    {
        var id = this._userTypesToShowOrder[i];
        children.push({
            id: id,
            text: this._userTypesToShow[id],
            cls: ["user-type-container"," ","user-type-",id,"-container"].join(''),
            singleClickExpand: true,
            expanded: false,
            listeners: { "expand": function(node) { self.identifyNode(node); } }       
        });
...
UserControl.prototype.createDialog = function() {
    var self = this;
    var root = new Ext.tree.AsyncTreeNode({
        text: "Root",
        id: "root",
        expanded: true,
        listeners: { "expand": function(node) { self.identifyNode(node); } },
        children: this.createUserTypeNodes()
    });
    var loader = new Ext.tree.TreeLoader({
        url: "slxdata.ashx/slx/crm/-/users/{0}",
        requestMethod: "GET"
    });
    loader.on("beforeload", function(treeLoader, node) {
        treeLoader.formatUrl = treeLoader.formatUrl || treeLoader.url;
    ...
    var dialog = new Ext.Window({
        id: this._clientId + "_window",
        title: UserControlResources.SlxUserControl_Header,
        cls: "lookup-dialog user-lookup-dialog",
        layout: "border",
        closeAction: "hide",
        plain: true,
        height: 400,
        ...
        items: [{
            ...
            buttons: [{
                id: this._clientId + "_ok",
                text: GetResourceValue(UserControlResources.SlxUserControl_Button_Ok, "OK"),
                handler: function() {
                    var node = tree.getSelectionModel().getSelectedNode();
                    if (!node || !node.isLeaf()) {
                        Ext.Msg.alert("", UserControlResources.SlxUserControl_UserType_NoRecordSelected);
                        return;
                    } 
                ...
                id: this._clientId + "_cancel",
                text: UserControlResources.SlxUserControl_Button_Cancel,
                handler: function() {
                    self.close();
                }
}],
                ...
                dialog.on("show", function(win) {
                    $(document).bind("keyup.UserControl", function(evt) {
                        switch (evt.keyCode) {
                            case 13: 
                            case 32: 
                                var node = tree.getSelectionModel().getSelectedNode();
                    ...
                    $(document).bind("keypress.UserControl", function(evt) {
                        var key = evt.charCode || evt.keyCode;
                        var text = String.fromCharCode(key);
                ...
                dialog.on("hide", function(win) {
                    $(document).unbind("keypress.UserControl");
                    $(document).unbind("keyup.UserControl");
                });
                tree.on("dblclick", function(node, evt) {
                    if (!node || !node.isLeaf())
                        return;
...
UserControl.prototype.search = function(node, expr) {
    if (node)
    {
        if (node.id != "root" && expr.test(node.text))
            return node;
...
UserControl.prototype.close = function() {
    if (this._dialog)
        this._dialog.hide();
};

UserControl.prototype.setResult = function(id, text) {
    this._result = {id: id, text: text}; 
...
UserControl.prototype.invokeChangeEvent = function(el) {
    if (document.createEvent)
    {
        //FF
        var evt = document.createEvent('HTMLEvents'); 
...
function UserPanel(clientId, autoPostBack)
{
    this.ClientId = clientId;
    this.UserDivId = clientId + "_outerDiv";
    this.UserTypeId = clientId + "_UserType";
    this.UserTextId = clientId + "_LookupText";
    this.UserValueId = clientId + "_LookupResult";
    this.ResultDivId = clientId + "_Results";
    this.UserListId = clientId + "_UserList";
    this.AutoPostBack = autoPostBack;
    this.LookupValue = "";
    this.DisplayValue = "";
    this.onChange = new YAHOO.util.CustomEvent("change", this);
...
function UserPanel_Show()
{
    if (this.CanShow())
    {
        if ((this.panel == null) || (this.panel.element.parentNode == null))
        {
            var lookup = document.getElementById(this.UserDivId);
            lookup.style.display = "block";
            this.panel = new YAHOO.widget.Panel(this.UserDivId, { visible:false, width:"305px",  fixedcenter:true, constraintoviewport:true, underlay:"shadow", draggable:true, modal:false });
            this.panel.render();
        }
        this.panel.show();
        var userType = document.getElementById(this.UserTypeId);
        var list = document.getElementById(this.UserListId);
        if (list.options.length < 1)
        {
            var type = userType.selectedIndex;
            if (type == -1) { type = 0; }
            var vURL = "SLXUserListHandler.aspx?usertype=" + type;
            if (typeof(xmlhttp) == "undefined") {
                xmlhttp = YAHOO.util.Connect.createXhrObject().conn;
            }
            xmlhttp.open("GET", vURL, false);
...
function UserPanel_Ok()
{
    ...
    this.DisplayValue = list.options[list.selectedIndex].innerHTML;
    var UserText = document.getElementById(this.UserTextId);
    var UserValue = document.getElementById(this.UserValueId);
    UserText.value = this.DisplayValue;
    UserValue.value = this.LookupValue;
    this.panel.hide();
    this.onChange.fire(this);
    this.InvokeChangeEvent(UserText);
    if (this.AutoPostBack)
    {
        if (Sys)
        {
...
UserPanel.prototype.CanShow = UserPanel_CanShow;
UserPanel.prototype.Show = UserPanel_Show;
UserPanel.prototype.Close = UserPanel_Close;
UserPanel.prototype.GetUserTypeParam = UserPanel_GetUserTypeParam;
UserPanel.prototype.Ok = UserPanel_Ok;
UserPanel.prototype.PerformLookup = UserPanel_PerformLookup;
UserPanel.prototype.getVal = UserPanel_getVal;
UserPanel.prototype.setVal = UserPanel_setVal;
UserPanel.prototype.InvokeChangeEvent = UserPanel_InvokeChangeEvent;
...
{
    this.ClientId = clientID;
    this.phoneID = clientID + "_phoneText";
    this.phone = new phoneProp(phone, this);
    this.AutoPostBack = autoPostBack;
    var elem = document.getElementById(this.phoneID);
    if (elem != null)
    {
...
{
    var elem = document.getElementById(this.phoneID); 
    elem.value = FormatPhoneNumber(val);
}

// This function automatically converts a string of text into a phone number
// It includes a translater of A-Z to corresponding keys
...
{
    var elem = document.getElementById(this.phoneID);
    this.phone.set(elem.value);
    this.phone.onChange.fire();
    if (this.AutoPostBack)
    {
        if (Sys)
        {
...
{
    this.value = val;
    this.parentElement.FormatPhone(val);
}
...
Phone.prototype.FormatPhone = Phone_FormatPhone;
Phone.prototype.FormatPhoneChange = Phone_FormatPhoneChange;

function PickList(clientId, picklistName, storeAsText, multiSelect, mustExistInList, alphaSort, autoPostback, canEditText, required) {
    this.clientId = clientId;
    ...
    this.defaultValueId = clientId + "_DefaultValue";
    this.defaultCodeId = clientId + "_DefaultCode";
    this.onChange = new YAHOO.util.CustomEvent("change", this);
    ...
    if(this.Required)
    {
       ...
       if(text.value == '')
       {
           ...
           var defaultValue = document.getElementById(this.defaultValueId);
           var defaultCode = document.getElementById(this.defaultCodeId);
           if(this.storeValueAsText)
           {
...
function PickList_SetVisibility(visible) {
    ...
    if(visible)
    {
        ...
        if (this.multiSelect != "True")
        {
            var pickListEl = document.getElementById(this.clientId);
            var pickListTextEl = document.getElementById(this.clientId + "_Text");
            var innerListEl = document.getElementById(this.clientId + "_InnerListDIV");                 
            var adjust = ( $(innerListEl).outerWidth() - $(innerListEl).width() );
            $(innerListEl).width($(pickListTextEl).outerWidth() - adjust);            
        }
...
function PickList_ResolveValue() {
    if((this.CanEditText)&&(this.mustExistInList)&&(this.multiSelect == "False"))
    {
       ...
       var codeELM = document.getElementById(this.codeId);
       var list = document.getElementById(this.listId);
       if(textELM.Value != "")
       {
          for (key = 0; key < list.options.length; key++) 
          {
             ...
             {
                textELM.value = list.options[key].text;
                codeELM.value = list.options[key].value; //This is the picklistItemId
                break;
              }
           }
        }   
...
function PickList_ButtonClick(e) {
    ...
    if( this.isDroppedDown ) {
        ...
        if (this.multiSelect == "True")
        {
            ...
            if (innerList)
            {                
                var container = $("#" + this.clientId).find(".picklist-items");
...
// this is the callback from the PickList_GetList() method
function PickList_HandleHttpResponse(results) {
    ...
    else
    {
        ...
        {
            if (items[i] == "") continue;
            var parts = PickList_GetItemParts(items[i]);
            var oOption = document.createElement("OPTION");
            list.options.add(oOption);
            if (parts[0].charAt(0) == '@')
            {
...
function PickList_SelectionChanged() {
    ...
    var list = document.getElementById(this.listId);
    if (list.options != undefined && list.options.length > 0)
    {
        var text = document.getElementById(this.textId);
        var code = document.getElementById(this.codeId);
        ...
        if (list.selectedIndex != -1)
        {
            ...
            if (oldText != newText)
            {
                ...
                {
                    if (navigator.appName.toLowerCase().indexOf("microsoft") == -1) // document.createEvent)
                        hyperText.textContent= newText; //Firefox
                    else
...
function PickList_MultiSelect() {
    ...
    for (var i=0; i < items.length; i++)
    {
        ...
        {
            if (hasMultiple) {
                newValue = newValue + ", " + this.multiSelectValues[items[i].id]; 
            }
            else
            {
                newValue = this.multiSelectValues[items[i].id];
                hasMultiple = true;
            }
        }       
    } 
    ...
    if (newValue == ""){
       // newValue = PickListResource.PickListControl_HyperText_NoValue; 
    }
    else{
       if(newValue.length > text.maxLength){  
          newValue = newValue.substring(0, text.maxLength-1); 
       }     
    }   
    this.SetVisibility(false);
    this.isDroppedDown = false;
    if (newValue != text.value) {
        ...
        {
            if (navigator.appName.toLowerCase().indexOf("microsoft") == -1) // document.createEvent)
                hyperText.textContent = newValue; //Firefox
            else
...
// if the user clicks on any key except those in the following list AND
// the CanEditText attribute is set to false, the keypress will be swallowed
// so they cannot enter any text into the field.
...
function PickList_TextChange(e) {
    ...
    if (e.keyCode == 13) //Enter
    {
        ...
        for( var i = 0; i < items.length; i++ ) {
            var item = items[i];
            if(unescape(item.text).toLowerCase().indexOf(unescape(text.value).toLowerCase() ) == 0 ) {
                list.selectedIndex = i;
                matchFound = true;
                this.LastValidText = text.value;
                if (!this.isDroppedDown)
                {
                    this.SetVisibility(true);
                    this.isDroppedDown = true;
...
function PickList_SetCheckedState(itemId, index, value) {
    ...
    else
    {
        ...
        this.multiSelectValues[checkbox.id] = unescape(value);
    }
...
PickList.prototype.MimicBlur = PickList_MimicBlur;
PickList.prototype.HandleKeyDown = Picklist_HandleKeyDown;
PickList.prototype.ResolveValue = PickList_ResolveValue;
String.prototype.trim = function() {
	return this.replace(/^\s+|\s+$/g,"");
}

var slxdatagrid = function(gridID) {
    this.gridID = gridID;
    ...
    if (tbl) {
        ...
        if ((tbl.rows.length > 0) && (tbl.rows[0].cells.length > 0)) {
            var cell = null;
            if (YAHOO.util.Dom.hasClass(tbl.rows[0], this.pagerclassname))
                cell = tbl.rows[1].cells[0];
            else
                cell = tbl.rows[0].cells[0];


            this.expandable = (YAHOO.util.Dom.hasClass(cell, this.expandclassname) ||
			                   YAHOO.util.Dom.hasClass(cell, this.expandnoiconclassname) ||
			                   YAHOO.util.Dom.hasClass(cell, this.collapseclassname) ||
			                   YAHOO.util.Dom.hasClass(cell, this.collapsenoiconclassname));
            // set up listeners for click events on row expanders
            if (this.expandable) {
                this.expandAllCell = cell;
                YAHOO.util.Event.removeListener(cell, "click", this.expandCollapseAll);
                YAHOO.util.Event.addListener(cell, "click", this.expandCollapseAll, this, true);
            }
        }
...
slxdatagrid.prototype.setSortIDs = function() {
	if ((this.table.rows.length > 0) && (this.HeaderRow)) {
		...
		for (var i = 0; i < this.HeaderRow.cells.length; i++) {
			var links = YAHOO.util.Dom.getElementsBy(this.returnTrue, "A", this.HeaderRow.cells[i]);
...
slxdatagrid.prototype.expandCollapseAll = function() {
	if (YAHOO.util.Dom.hasClass(this.expandAllCell, this.expandclassname)) {
		//expand all
		for (var i = 1; i < this.table.rows.length; i++) {
			this.expandRow(this.table.rows[i]);
		}
		YAHOO.util.Dom.removeClass(this.expandAllCell, this.expandclassname);
		YAHOO.util.Dom.addClass(this.expandAllCell, this.collapseclassname);
	} else if (YAHOO.util.Dom.hasClass(this.expandAllCell, this.collapseclassname)) {
		//collapse all
		for (var i = 1; i < this.table.rows.length; i++) {
			this.collapseRow(this.table.rows[i]);
		}
		YAHOO.util.Dom.removeClass(this.expandAllCell, this.collapseclassname);
		YAHOO.util.Dom.addClass(this.expandAllCell, this.expandclassname);
	}
}
...
slxdatagrid.prototype.toggleRow = function(row) {
	if (row) {
		if (YAHOO.util.Dom.hasClass(row.cells[0], this.expandclassname) ||
		    YAHOO.util.Dom.hasClass(row.cells[0], this.expandnoiconclassname)) {
			this.expandRow(row);
		} else if (YAHOO.util.Dom.hasClass(row.cells[0], this.collapseclassname) ||
		           YAHOO.util.Dom.hasClass(row.cells[0], this.collapsenoiconclassname)) {
...
slxdatagrid.prototype.expandRow = function(row) {
	if (row) {
		if (YAHOO.util.Dom.hasClass(row.cells[0], this.collapseclassname) || (!YAHOO.util.Dom.hasClass(row.cells[0], this.expandclassname) && !YAHOO.util.Dom.hasClass(row.cells[0], this.expandnoiconclassname))) {
			return;
		}
		...
		for (var i = this.getColIndexStart(); i < row.cells.length; i++) {
			...
			if (cell.childNodes[0]) {
				if (YAHOO.util.Dom.hasClass(cell.childNodes[0], this.contentdivclassname)) {
					if (cell.childNodes[0].scrollHeight > collapseH) {
						YAHOO.util.Dom.setStyle(cell.childNodes[0], "height", "100%");
					}
				}
			}
		}
		...
		if (YAHOO.util.Dom.hasClass(row.cells[0], this.expandclassname))
		    YAHOO.util.Dom.replaceClass(expandoCell, this.expandclassname, this.collapseclassname);
	    else
...
slxdatagrid.prototype.collapseRow = function(row) {
	if (row) {
		if (YAHOO.util.Dom.hasClass(row.cells[0], this.expandclassname) || (!YAHOO.util.Dom.hasClass(row.cells[0], this.collapseclassname) && !YAHOO.util.Dom.hasClass(row.cells[0], this.collapsenoiconclassname))) {
			return;
		}
		var collapseH = this.collapsedheight.replace("px", "");
		var setH = collapseH;
		for (var i = this.getColIndexStart(); i < row.cells.length; i++) {
			...
			if (cell.childNodes[0]) {
				if (YAHOO.util.Dom.hasClass(cell.childNodes[0], this.contentdivclassname)) {
					...
					YAHOO.util.Dom.setStyle(cell.childNodes[0], "height", setH + "px");
				}
			}
		}
		var expandoCell = row.cells[0];
		
		if (YAHOO.util.Dom.hasClass(row.cells[0], this.collapseclassname))
		    YAHOO.util.Dom.replaceClass(expandoCell, this.collapseclassname, this.expandclassname);
		else
...
//window.status = this.gridID + " - initColWidths";
//debugger;
    if (this.HeaderRow) {
...
slxdatagrid.prototype.fillSpace = function() {
    if (this.HeaderRow) {
        ...
        var cont = YAHOO.util.Dom.get(this.containerId);
        var container = YAHOO.util.Region.getRegion(cont);
        var containerW = container.right - container.left;

        var tbl = YAHOO.util.Region.getRegion(this.table);

        this.doResize();

        var tblW = tbl.right - tbl.left;
        ...
        if (increaseBy > 3) {
            ...
            for (var i = start; i < this.HeaderRow.cells.length; i++) {
                ...
                if (this.wids[i]) {
                    ...
                    newtableregion = YAHOO.util.Region.getRegion(this.table);
...
//window.status = this.gridID + " - getCurrentWidths";
...
slxdatagrid.prototype.getColumnWidth = function(colIndex) {
    if (this.HeaderRow) {
	    if (this.HeaderRow.cells[colIndex]) {
		    var region = YAHOO.util.Region.getRegion(this.HeaderRow.cells[colIndex]);
...
//window.status = this.gridID + " - getWidthsFromCookie";

	var widthcookie = getCookie("GRIDCW");
	if (widthcookie) {
...
//window.status = this.gridID + " - setWidthsToCookie";
...
//window.status = this.gridID + " - generateWidthCookieString";
...
//window.status = this.gridID + " - setWidth - " + colIdx + " " + width;
	...
	if (this.HeaderRow) {
		for(var r = 0; r < this.table.rows.length; r++) {
			if (this.table.rows[r].cells[colIdx]) {
				...
				YAHOO.util.Dom.setStyle(this.table.rows[r].cells[colIdx], "width", width);
			}
		}
		if (persist) {
...
slxdatagridcolumn = function(headerColId, datagrid, colIdx, config) {
	if (headerColId) {
		this.datagrid = datagrid;
		this.colIndex = colIdx;
		this.headerColId = headerColId;
		this.init(headerColId, headerColId, config);
		this.handleElId = headerColId;
		this.setHandleElId(headerColId);
	
		var elem = YAHOO.util.Dom.get(headerColId); 
		if (elem) {	
			YAHOO.util.Event.removeListener(elem, "mouseover", this.onMouse);
			YAHOO.util.Event.addListener(elem, "mouseover", this.onMouse, this, true);
			YAHOO.util.Event.removeListener(elem, "mousemove", this.onMouse);
			YAHOO.util.Event.addListener(elem, "mousemove", this.onMouse, this, true);
			YAHOO.util.Event.removeListener(elem, "mouseout", this.onMouseOut);
			YAHOO.util.Event.addListener(elem, "mouseout", this.onMouseOut, this, true);
		}
	}
}

YAHOO.extend(slxdatagridcolumn, YAHOO.util.DragDrop);


slxdatagridcolumn.prototype.dispose = function() {
	this.datagrid = null;
	var elem = YAHOO.util.Dom.get(this.headerColId); 
	if (elem) {	
		YAHOO.util.Event.purgeElement(elem);
	}
}

slxdatagridcolumn.prototype.onMouseDown = function(e) {
...
slxdatagridcolumn.prototype.onDrag = function(e) {
	var newPos = YAHOO.util.Event.getPageX(e);
	var offsetX = newPos - this.startPos;
	var newWidth = Math.max(this.startWidth + offsetX, 30);
	var panel = this.getEl();
	panel.style.width = newWidth + "px";
	this.datagrid.setWidth(this.colIndex, newWidth + "px", true); 
}


slxdatagridcolumn.prototype.onMouse = function(e) {
	eventX = YAHOO.util.Event.getPageX(e);
	var elem = YAHOO.util.Dom.get(this.headerColId); 
	var region = YAHOO.util.Region.getRegion(elem);
	hotX = region.right - 10;
	if (eventX > hotX) {
		YAHOO.util.Dom.setStyle(this.headerColId, "cursor", "col-resize");
	} else {
		YAHOO.util.Dom.setStyle(this.headerColId, "cursor", "");
	}
}

slxdatagridcolumn.prototype.onMouseOut = function(e) {
	YAHOO.util.Dom.setStyle(this.headerColId, "cursor", "");
}

// ----------------------- end column resize section---------------------------------------------------------------------- //

slxdatagrid.prototype.doResize = function() {
    var cont = document.getElementById(this.containerId);
    var container = YAHOO.util.Region.getRegion(cont);
    ...
    if ((tbl.right - tbl.left) > (container.right - container.left)) {
       if (cont.style.height == "") {
            ...
            document.getElementById(this.containerId).style.height = (tblheight + 20) + "px";
        }
    }
}
...
Sage.SalesLogix.Controls.LiveGrid = function(options) {
    this._options = options;
    this._connections = {};
    this._maps = {};
    this._grid = null;
    this._summaryGrid = null;
    this._layout = {};
    
    this._stateId = this._options.id;
    
    this._requestedMetaData = false;
    this._summaryViewMgr = null;
    this._activeGrid = null;

    var cons = this._options.connections;
    for (var i = 0; i < cons.length; i++)
        this._connections[cons[i]["for"]] = cons[i];

    var maps = this._options.maps;
    for (var i = 0; i < maps.length; i++)
        this._maps[maps[i]["for"]] = maps[i];
    ...
    this.NumericGroupingChar = ",";
    var contextservice = Sage.Services.getService("ClientContextService");
    if ((contextservice) && (contextservice.containsKey("GroupingChar"))) {
        this.NumericGroupingChar = contextservice.getValue("GroupingChar");
    }
};

Ext.extend(Sage.SalesLogix.Controls.LiveGrid, Ext.util.Observable);

Sage.SalesLogix.Controls.LiveGrid.prototype.getOptions = function() { return this._options; };
Sage.SalesLogix.Controls.LiveGrid.prototype.getLayout = function() { return this._layout; };
//Sage.SalesLogix.Controls.LiveGrid.prototype.getSummaryLayout = function() { return this._summaryLayout; };
Sage.SalesLogix.Controls.LiveGrid.prototype.getConnections = function() { return this._connections; };
Sage.SalesLogix.Controls.LiveGrid.prototype.getMaps = function() { return this._maps; };
Sage.SalesLogix.Controls.LiveGrid.prototype.getNativeGrid = function() { return this.getGrid(); };
Sage.SalesLogix.Controls.LiveGrid.prototype.getGrid = function() { return (this._activeGrid) ? this._activeGrid : this._grid; };
Sage.SalesLogix.Controls.LiveGrid.prototype.getStateId = function() { return this._stateId; };
Sage.SalesLogix.Controls.LiveGrid.prototype.setStateId = function(value) { this._stateId = value; };
Sage.SalesLogix.Controls.LiveGrid.prototype.getId = function() { return this._options.id; };
Sage.SalesLogix.Controls.LiveGrid.prototype.clearMetaData = function() { this._requestedMetaData = false; };
Sage.SalesLogix.Controls.LiveGrid.prototype.hasStaticLayout = function() { return (this._options.columns && this._options.columns.length > 0); };

Sage.SalesLogix.Controls.LiveGrid.prototype.getStatePageId = function() { return this._options.id + "_" + Sage.Services.getService('ClientGroupContext').getContext().CurrentFamily; };

Sage.SalesLogix.Controls.LiveGrid.prototype.getSelectionsTotal = function(entity) {
    var selections;
    var activeGrid;
    var selectCount;
    activeGrid = this.getGrid();
    selections = activeGrid.getSelections(); 
    selectCount = selections.length;
    return selectCount;
};

Sage.SalesLogix.Controls.LiveGrid.prototype.getSelectionsByID = function(entity) {
    var arraySelectionIDs = new Array;
    var activeGrid;
    var selections;
    activeGrid = this.getGrid();
    selections = activeGrid.getSelections(); 
    ...
    for (i = 0; i < count; i++)
    {
        var id = selections[i].data[GetKeyField(entity)] || selections[i].data[entity + "_ID"];
        arraySelectionIDs[i] = id;     
    }        
    return arraySelectionIDs;
};

Sage.SalesLogix.Controls.LiveGrid.prototype.getTotalCount = function() {
    
    var activeGrid;
    var totalCount;
    activeGrid = this.getGrid();
    totalCount = activeGrid.getStore().getTotalCount();   

    return totalCount;
};

Sage.SalesLogix.Controls.LiveGrid.prototype.init = function() {
    //    if (typeof Sage.Services != 'undefined')
    //    {
    //        var grpContext = Sage.Services.getService("ClientGroupContext");
    //        if (grpContext) {
    //            var tbl = grpContext.getContext().CurrentTable;
    //            this._summaryLayout = {
    ...
    //        }
    //    }
    if (this.hasStaticLayout()) {
        this._layout = { entity: this._options.entity, columns: this._options.columns };
        this.processLayout();
    }
    this.clearMetaData();
    this.initGrid();
...
Sage.SalesLogix.Controls.LiveGrid.prototype.buildSummaryColumnModel = function() {
    var model = [];
    var grpContext = Sage.Services.getService("ClientGroupContext");
    if (grpContext) {
        var tbl = grpContext.getContext().CurrentTable;
        var item = {
            header : grpContext.CurrentName,
            align : 'left',
            width : 'auto',
            sortable : false,
            dataIndex : GetKeyField(tbl),
            renderer : SummaryRenderer
        }
        model.push(item);
    }
    return new Ext.grid.ColumnModel(model);
};

Sage.SalesLogix.Controls.LiveGrid.prototype.buildColumnModelConfig = function() {
    var model = [];
...
Sage.SalesLogix.Controls.LiveGrid.prototype.buildColumnModel = function() {
    if (this.hasStaticLayout())
        return new Ext.grid.ColumnModel(this.buildColumnModelConfig());
    else
        return new Ext.grid.ColumnModel([]);
};

Sage.SalesLogix.Controls.LiveGrid.prototype.buildSummaryReader = function() {
    var columns = [];
    var idname = "ACCOUNTID";
    var grpContext = Sage.Services.getService("ClientGroupContext");
    if (grpContext) {
        idname = GetKeyField(grpContext.getContext().CurrentTable);
    }
    columns.push({ name: idname, sortType: "string" });
    columns.push({name: 'SLXRN', sortType: 'int'});
    return new Ext.ux.data.BufferedJsonReader({
...
Sage.SalesLogix.Controls.LiveGrid.prototype.buildReader = function() {
    var columns = [];
    if (this._options.useMetaData === false) {
        columns.push({ name: 'SLXRN', sortType: 'int' });
        for (var i = 0; i < this._layout.columns.length; i++) {
            item = {
                name: this._layout.columns[i].alias,
                sortType: (this._layout.columns[i].sortType) ? this._layout.columns[i].sortType : "string"
            };

            switch (this._layout.columns[i].format) {
                case "Owner":
                case "User":
                    columns.push({ name: this._layout.columns[i].alias + "NAME", sortType: "string" });
                    break;
                case "DateTime":
                    item.sortType = "int";
                    break;
                case "PickList Item":
                    columns.push({ name: this._layout.columns[i].alias + "TEXT", sortType: "string" });
                    break;
            }

            if (this._layout.columns[i].isWebLink)
                columns.push({ name: GetKeyField(this._layout.entity), sortType: "string" });

            if (this._layout.columns[i].alias == "ACCOUNT" || this._layout.columns[i].alias == "ACCOUNT_ACCOUNT")
                columns.push({ name: "ACCOUNT_ID", sortType: "string" });
        ...
        if (this._maps["data"])
            return new Ext.ux.data.BufferedJsonReader({
                root: this._maps["data"].data,
                versionProperty: this._maps["data"].version,
                totalProperty: this._maps["data"].count
            }, columns);
        else
            return new Ext.ux.data.BufferedJsonReader({
                root: "response.value.items",
...
Sage.SalesLogix.Controls.LiveGrid.prototype.buildSummaryDataStore = function(reader) {
    var store = new Ext.ux.grid.BufferedStore({
        //autoLoad : true,
        autoLoad : false,
        bufferSize: 1000,
        ...
        filter: { },
        url: "builder",
        buildUrl: this._connections["summary"]
    });
    
    store.on("beforeload", function (store, options) {
            this._connections["summary"].parameters["meta"] = "true";
            var url = this.buildUrl("summary", {instance: self, store: store, startAt: 0, args: arguments});
            store.proxy.conn.url = url;
            store.proxy.conn.method = "GET";
            store.url = url;
        }, this);
    
    this.fireEvent('createsummarystore', this, store);    
    
    return store;
};

Sage.SalesLogix.Controls.LiveGrid.prototype.buildDataStore = function(reader) {
    var store = new Ext.ux.grid.BufferedStore({
        //autoLoad: true,
        autoLoad : false,
        bufferSize: 150,
        ...
        filter: {},
        url: "builder",
        buildUrl: this._connections["data"]
    });
    
    store.on("beforeload", function (store, options) {
            if (this._options.useMetaData && !this._requestedMetaData)
                if (this._connections["data"])
                    this._connections["data"].parameters["meta"] = "true";
    ...
    store.on("load", function(store, records, options) {            
            if (this._options.useMetaData)
                if (this._connections["data"])
                {
                    delete this._connections["data"].parameters["meta"];
                    this._requestedMetaData = true;                    
                }
    ...
    {  
        store.on("metachange", function(store, meta) {            
                args = {data: meta, layout: null};
                this.fireEvent("layout", this, args);
                ...
                {
                    this._layout = {entity: meta.layout.mainTable, columns: []};                                          
                    for (var i = 0; i < meta.layout.items.length; i++)                    
                        this._layout.columns.push({
                            dataPath: meta.layout.items[i].dataPath,
                            alias: meta.layout.items[i].alias,
                            align: meta.layout.items[i].align,
                            formatString: meta.layout.items[i].formatString,
                            format: meta.layout.items[i].format,
                            caption: meta.layout.items[i].caption,
                            captionAlign: meta.layout.items[i].captionAlign,
                            sortType: null,
                            width: meta.layout.items[i].width,
                            isVisible: ((typeof meta.layout.items[i].visible === "boolean") ? meta.layout.items[i].visible : !!meta.layout.items[i].visible.match(/T/i)),
                            isWebLink: ((typeof meta.layout.items[i].webLink === "boolean") ? meta.layout.items[i].webLink : !!meta.layout.items[i].webLink.match(/T/i)),
                            isPrimaryKey: false                        
                        }); 
                }                                
                this.processLayout();
                this._grid.getColumnModel().setConfig(this.buildColumnModelConfig());
            }, this);
    }
        
    this.fireEvent('createstore', this, store);
        
    return store;
};

Sage.SalesLogix.Controls.LiveGrid.prototype.buildView = function() {
    var view = new Ext.ux.grid.BufferedGridView({
        nearLimit: 25,
        forceFit: (this.getOptions().forceFitColumns === false) ? false : true, 
        autoFill: true,
        loadMask: {
            msg: SlxLiveGridResources.PleaseWait
        }
    }); 
    
    view.on("beforebuffer", function (view, store, rowIndex, visibleRows, totalCount, bufferOffset) {    
    ...
    this.fireEvent('createview', this, view);

    return view;
};

Sage.SalesLogix.Controls.LiveGrid.prototype.buildSummaryView = function() {
    var view = new Ext.ux.grid.BufferedGridView({
        nearLimit: 25,
        forceFit: true,
        autoFill: true,
    ...
    view.on("beforebuffer", function (view, store, rowIndex, visibleRows, totalCount, bufferOffset) {    
            var url = this.buildUrl("summary", {instance: self, store: store, startAt: bufferOffset, args: arguments});           
            store.proxy.conn.url = url;
            store.proxy.conn.method = "GET";
            store.url = url;
        }, 
    ...
    this.fireEvent('createsummaryview', this, view);
        
    return view;
};

Sage.SalesLogix.Controls.LiveGrid.prototype.addToPanel = function(panel) {
    if (!panel && this._targetPanel)
        panel = this._targetPanel;

    if (!panel)
        return;
        
    $(panel.getEl().dom).find(".x-panel-body").children().hide();
    panel.add(this._grid);
    if (this._summaryViewMgr != null) {
        panel.add(this._summaryGrid);
    }
    
    var initialView = Ext.state.Manager.get(this.getStatePageId(), this.LIST); 

    var initIndex = 0;
    this._activeGrid = this._grid;
    if (initialView == this.SUMMARY) {
        this._activeGrid = this._summaryGrid;
        initIndex = 1;
    }
    
    panel.layout.setActiveItem(initIndex);
    panel.doLayout();
};

Sage.SalesLogix.Controls.LiveGrid.prototype.initGrid = function() {
    var self = this;

    this.fireEvent("beforeinit", this, {});
    
    var reader = this.buildReader();    
    var store = this.buildDataStore(reader);
    var view = this.buildView();
    var columnModel = this.buildColumnModel();
    var selectionModel = new Ext.ux.grid.BufferedRowSelectionModel({singleSelect: this._options.singleSelect});        
    var toolbar = new Ext.ux.BufferedGridToolbar({
        view: view,
        id:this._id + "_listtbar",
        refreshText: SlxLiveGridResources.Refresh,
        displayInfo : true
    });
    
    toolbar.displayMsg = SlxLiveGridResources.DisplayMessage;
    toolbar.emptyMsg = SlxLiveGridResources.EmptyMessage;
    toolbar.refreshText = SlxLiveGridResources.Refresh;
    
    var panel;
    if (typeof mainViewport != 'undefined')
            var viewport = mainViewport; 
            
    if (typeof this._options.target === "object")
    {
    ...
    if (panel) {    
        ...
        panel.el.dom.oncontextmenu = function(){ return false };
    }
    
    var config = {
        id: this._options.id + "_grid",
        ds: store,
        enableDragDrop: false,
        cm: columnModel,
        sm: selectionModel,
        loadMask: {
            msg: SlxLiveGridResources.Loading
        },
        view: view,
        title: "",
        stripeRows: true,
    ...
    if (!panel && !this._options.createOnly)
        config.renderTo = this._options.target;
       
    this._grid = new Ext.grid.GridPanel(config);
    if (typeof showAdHocMenu != 'undefined') {
        this._grid.addListener('contextmenu', showAdHocMenu);
    }
    
    this.fireEvent('creategrid', this, this._grid);
    
    var summaryGrid = {};
    if (this._summaryViewMgr != null) {
        var summaryReader = this.buildSummaryReader();  
        var summaryStore = this.buildSummaryDataStore(summaryReader);
        var summaryView = this.buildSummaryView();
        var summaryColumnModel = this.buildSummaryColumnModel();
        var summarySelectionModel = new Ext.ux.grid.BufferedRowSelectionModel();    
        var toolbar = new Ext.ux.BufferedGridToolbar({
            view: summaryView,
            id:this._id + "_summtbar",
            refreshText: SlxLiveGridResources.Refresh,
            displayInfo : true
        });
        ...
        var summaryConfig = {
            id: this._options.id + "_Summary",
            ds: summaryStore,
            enableDragDrop: false,
            cm: summaryColumnModel,
            sm: summarySelectionModel,
            loadMask: {
                msg: SlxLiveGridResources.Loading
            },
            view: summaryView,
            ...
            stripeRows: false,
            tbar: toolbar,
            stateId: this._stateId + "_Summary",
            border: false
        };
        summaryGrid = new Ext.grid.GridPanel(summaryConfig);
        this._summaryGrid = summaryGrid;
    }               
    ...
    if (panel && !this._options.createOnly) 
    {
        ...
        var tbar = this._grid.getTopToolbar();
        tbar.addFill();
        
        if (this._summaryViewMgr != null) {
            tbar.addButton({
                text: SlxLiveGridResources.List,
                handler: this.showListView,
                scope: self,
                disabled: true
            });
            tbar.addButton({
                text: SlxLiveGridResources.Summary,
                handler: this.showSummaryView,
                scope: self        
            });
            
            var summarytbar = this._summaryGrid.getTopToolbar();
            summarytbar.addFill();
            summarytbar.addButton({
                text: SlxLiveGridResources.List,
                handler: this.showListView,
                scope: self
            });
            ...
            summarytbar.addButton({
                icon : "ImageResource.axd?scope=global&type=Global_Images&key=Help_16x16",
                handler : function() { linkTo(encodeURI("help.aspx?content=help*\\base\\summaryview.aspx"), true); },
                text : "",
                cls:"x-btn-text-icon"         
            });
        }
        ...
        var self = this;
        if (self._options.helpurl != "") {
            tbar.addButton({
                icon : "ImageResource.axd?scope=global&type=Global_Images&key=Help_16x16",
                handler : function() { linkTo(encodeURI(self._options.helpurl), true); },
                text : "",
                cls:"x-btn-text-icon"
            });
        }
        
        if (Ext.state.Manager.get(this.getStatePageId(), this.LIST) == this.LIST) {
            store.load();
        } else {
            summaryStore.load();
        }
...
Sage.SalesLogix.Controls.LiveGrid.__urlBuilders = { 
...
Sage.SalesLogix.Controls.LiveGrid.prototype.buildUrl = function(which, context) {
    return Sage.SalesLogix.Controls.LiveGrid.__urlBuilders[this._connections[which].builder](this._connections[which], context);    
};

Sage.SalesLogix.Controls.LiveGrid.prototype.processLayout = function() {

    var l = this._layout.columns.length;
    for (var i = 0; i < l; i++)
    {
        var column = this._layout.columns[i];
        if (column.isVisible == false)
            continue;
...
Sage.SalesLogix.Controls.LiveGrid.prototype.createEntityLinkRenderer = function(column) {
    var self = this;
    ...
    if (typeof column === "string")
        ...
        entity = (column.dataPath.lastIndexOf("!") > -1)
            ...
            : column.dataPath.substring(0, column.dataPath.lastIndexOf(":"));
    var tableName = entity.slice(0);
    var keyField = GetKeyField(tableName);
    var grpService = Sage.Services.getService("ClientGroupContext");
    if (grpService) {
        if (entity == grpService.getContext().CurrentTable) entity = grpService.getContext().CurrentEntity;
    }
    return function(value, meta, record, rowIndex, columnIndex, store) {
        var id = record.data[keyField] || record.data[tableName + "_ID"];
        if (!id)
        ...
        return String.format("<a href={2}.aspx?entityid={0}>{1}</a>", id, Ext.util.Format.htmlEncode(value), entity);
    };
};

Sage.SalesLogix.Controls.LiveGrid.prototype.createFixedNumberRenderer = function(column) {
    var format = column.formatString.replace(/%.*[dnf]/, "{0}").replace("%%", "%");
    var useGroupingChar = (column.formatString.match(/%.*[dnf]/) == null) ?
        false : // is not a valid format string
        (column.formatString.match(/%.*[dnf]/)[0].indexOf("n") > 0);
    ...
    var NumericGroupingChar = this.NumericGroupingChar;
    var rx = /(\d+)(\d{3})/;
    if (precision && !isNaN(precision)) {
        return function(value, meta, record, rowIndex, columnIndex, store) {
            if (value && !isNaN(value)) {
...
Sage.SalesLogix.Controls.LiveGrid.prototype.createDateTimeRenderer = function(column) {
    var renderer = Ext.util.Format.dateRenderer(ConvertToPhpDateTimeFormat(column.formatString));
    return function(value, meta, record, rowIndex, columnIndex, store) {
        return renderer(value);    
    };
...
Sage.SalesLogix.Controls.LiveGrid.prototype.createPhoneRenderer = function(column) {
    return LiveGrid_RenderPhoneNumber;
};

function LiveGrid_RenderPhoneNumber(value, meta, record, rowIndex, columnIndex, store) {
    if ((value == null) || (value == "null")) return "&nbsp;";
    if (!value || value.length != 10)
        return Ext.util.Format.htmlEncode(value);
...
Sage.SalesLogix.Controls.LiveGrid.prototype.createEmailRenderer = function(column) {
    return function(value, meta, record, rowIndex, columnIndex, store) {
        return (value ? String.format("<a href=mailto:{0}>{0}</a>", Ext.util.Format.htmlEncode(value)) : "");
    };    
};

Sage.SalesLogix.Controls.LiveGrid.prototype.createDefaultRenderer = function(column) {
    return function(value, meta, record, rowIndex, columnIndex, store) {
        return (value ? Ext.util.Format.htmlEncode(value) : "&nbsp;");
    }
}

var summaryrenderedindex = 0;
function SummaryRenderer(value, meta, record, rowIndex, columnIndex, store) {
//alert('Summary Renderer: ' + value);
    if (!value) { return "&nbsp;"; }
    if (value.length != 12) { return value; }
    var contentEl = value + "_" + summaryrenderedindex++;
    window._SummaryViewManager.getData(value, contentEl);
    return String.format("<div id='{0}' style='min-height:110px'>{1}</div>", contentEl, SlxLiveGridResources.Loading);
};

Sage.SalesLogix.Controls.LiveGrid.prototype.showSummaryView = function(btn, pressed) {
    var panel;
    if (mainViewport)
        panel = mainViewport.findById(this._options.target);
    if (panel) {
        panel.layout.setActiveItem(1);
        this._activeGrid = this._summaryGrid;
        //var store = this._summaryGrid.getStore();
        //store.load();
        Ext.state.Manager.set(this.getStatePageId(), this.SUMMARY);
        this.fireEvent("toggleview", [this, {}]);
    }
};

Sage.SalesLogix.Controls.LiveGrid.prototype.showListView = function(btn, pressed) {
    var panel;
    if (mainViewport)
        panel = mainViewport.findById(this._options.target);
    if (panel) {
        ...
        //store.load();
        Ext.state.Manager.set(this.getStatePageId(), this.LIST);
        this.fireEvent("toggleview", [this, {}]);
    }
};
...
function GetKeyField(tableName) {
    var keyField = tableName + "ID";
    var grpService = Sage.Services.getService("ClientGroupContext");
    if ((grpService) && (grpService.getContext().CurrentTable == tableName)) {
        keyField = grpService.getContext().CurrentTableKeyField;
    }
    return keyField;
}

function makeEntityRenderer(entity) {
    //return function(e,p,r){return r.data[displayvalue]};
    var keyField = GetKeyField(entity);
    return function(e,p,r){
        var id = r.data[keyField];
        if ((typeof id == "undefined") || (id == ""))
            id = r.data[entity+"_ID"];
        if ((typeof id == "undefined") || (id == ""))
            return e;
        return String.format("<a href={2}.aspx?entityid={0}>{1}</a>", id, e, entity);
    }
};

function makeFixedNumRenderer(strformat) {
    var width;
    var precision;                    
    if (strformat.indexOf(".") > -1) {
        width = strformat.charAt(strformat.indexOf(".")-1);
        precision = strformat.charAt(strformat.indexOf(".")+1);                    
    } else {
        width = strformat.charAt(strformat.indexOf("%")+1);
    }
    var sformat = strformat.replace(/%.*[dnf]/, "{0}").replace("%%", "%");
    if (!isNaN(precision)) {
        return function(e){
            if ((!isNaN(e)) && (e != "")) {
                return String.format(sformat, e.toFixed(precision));
            } else {
                return e;
            }
        };
...
//-- Sage.SalesLogix.Controls.SummaryViewChildWindowManager ----------------------------------------------------------------------------------------------------------------------------------

Sage.SalesLogix.Controls.SummaryViewChildWindowManager = function() {
    this.containerID = 'SummaryViewChildDataBox';
    var box = document.getElementById("SummaryViewChildDataBox");
    if (!box) {
        $("body").append("<div id=\"SummaryViewChildDataBox\"></div>");
    }
    this.dataMgr = new Sage.SalesLogix.Controls.SummaryDataManager();
    this.timeoutpointers = new Array();

    this.childdatatemplates = { };
    this.whereformatstrings = { };
    this.win = new Ext.Window( {
        contentEl:'SummaryViewChildDataBox',
        id:this.containerID + "_win",
        layout :'fit',
        width : 300,
        height : 150,
        resizable:true,
...
Sage.SalesLogix.Controls.SummaryViewChildWindowManager.prototype.showMyData = function(queryid, entityid, winTitle, e) {
    if (!entityid) {
        return;//alert("Error: Missing entity id");
    }
    var fmtstr = this.whereformatstrings[queryid];
    if (!fmtstr) { 
        //alert("Error: Missing filter format string: " + queryid); 
        return; 
    }
...
Sage.SalesLogix.Controls.SummaryViewChildWindowManager.prototype.receiveChildData = function(context, data) {
    var self = context.childWinMgr;
    var template = self.childdatatemplates[context.name];
    template.overwrite(self.containerID, data);
    self.win.setTitle(context.winTitle);
    self.cancelSummaryClose();
    self.win.show();
    if (context.x) {
        self.win.setPosition(context.x, context.y);
    }

    if (doOnce) {
        //var elem = Ext.get("SummaryViewChildDataBox_win");
        var elem = self.win.getEl();
        if (elem) {
            elem.addListener("mouseover", self.cancelSummaryClose, self );
            elem.addListener("mouseout", self.mouseOut, self );
        }
        doOnce = false;
    }
}

Sage.SalesLogix.Controls.SummaryViewChildWindowManager.prototype.handleDataError = function(context, err) {
    if (context.childWinMgr.win.isVisible())
        context.childWinMgr.win.hide();
}

Sage.SalesLogix.Controls.SummaryViewChildWindowManager.prototype.mouseOut = function(e) {
    this.timeoutpointers.push(window.setTimeout(this.closeSummaryWin, 1000));
}

Sage.SalesLogix.Controls.SummaryViewChildWindowManager.prototype.closeSummaryWin = function() {
    if (window._SummaryViewManager.childWinMgr.win.isVisible())
        window._SummaryViewManager.childWinMgr.win.hide();
}

Sage.SalesLogix.Controls.SummaryViewChildWindowManager.prototype.cancelSummaryClose = function() {
    var self = window._SummaryViewManager.childWinMgr;
    for (var i = self.timeoutpointers.length - 1; i > -1; i--) {
        window.clearTimeout(self.timeoutpointers[i]);
        self.timeoutpointers.pop;
    }
}



Sage.SalesLogix.Controls.SummaryViewManager = function() {
    this.summarycardtemplate = { };
    this.dataMgr = { };
    this.queryname = '';
    this.whereformatstring = '';
    this.childWinMgr = { };
    this.stringresources = { };

}
Sage.SalesLogix.Controls.SummaryViewManager.prototype.showChildList = function(queryid, entityid, winTitle, e) {
    if (!e) { var e = window.event; }
    this.childWinMgr.showMyData(queryid, entityid, winTitle, e);
}

Sage.SalesLogix.Controls.SummaryViewManager.prototype.getData = function(entityId, contentEl) {
    window.setTimeout(String.format("window._SummaryViewManager.getDataRealityCheck('{0}','{1}')", entityId, contentEl), 250);
}

Sage.SalesLogix.Controls.SummaryViewManager.prototype.getDataRealityCheck = function(entityId, contentEl) {
    var elem = document.getElementById(contentEl); 
    if (elem) {
        var self = window._SummaryViewManager;
        var options = {
            name: self.queryname,
            where: String.format(self.whereformatstring, entityId)
        }
        var context = {
            name: self.queryname,
            viewmgr: self,
...
Sage.SalesLogix.Controls.SummaryViewManager.prototype.receiveData = function(context, data) {
    var self = context.viewmgr;
    var contentEl = context.contentEl;
    if (document.getElementById(contentEl)) {
        self.summarycardtemplate.overwrite(contentEl, data);
    }
}
...
Sage.SalesLogix.Controls.SummaryViewManager.prototype.navToDetail = function(entityid, tabName, e) {
    if ((entityid == "") || (entityid.length != 12)) { return; }
    var url = document.location.href;
    if (url.indexOf("?") > -1) {
        url = url.substring(0, url.indexOf("?"));
...
Sage.SalesLogix.Controls.SummaryDataManager = function() {
    this.rooturl = 'slxdata.ashx/slx/crm/-/namedqueries?format=json&';
}

Sage.SalesLogix.Controls.SummaryDataManager.prototype.requestData = function(params, successcallback, errorcallback, context) {     
    ...
    var url = this.rooturl + this.buildQParamStr(params);
    $.ajax({
        url: url,
        dataType: 'json',
        success: function(data) {
...
Sage.SalesLogix.Controls.SummaryDataManager.prototype.buildQParamStr = function(params) {
    var o = params;
    var p = [];       
    if (typeof o === "object")
        for (var k in o)
...
Sage.GroupManagerService = function() {
    this._contextService = Sage.Services.getService("ClientGroupContext");    
    this._saveFilterTimeouts = {}
    this._filters = {};
    this._listeners = {};
    this._listeners[Sage.GroupManagerService.FILTER_CHANGED] = [];   
    this._listeners[Sage.GroupManagerService.CURRENT_GROUP_CHANGED] = [];
    this._listeners[Sage.GroupManagerService.LISTENER_HOLD_STARTED] = [];
    this._listeners[Sage.GroupManagerService.CURRENT_GROUP_POSITION_CHANGED] = [];
    this._listeners[Sage.GroupManagerService.CURRENT_GROUP_COUNT_CHANGED] = [];
    
    //should this be here? or in the ClientGroupContextService?
    var self = this;
    $(document).ready(function() {        
        var prm = Sys.WebForms.PageRequestManager.getInstance();
        prm.add_beginRequest(function(sender, args) {
            //attach old context data to the request
            var request = args.get_request();       
            request.__originalGroupContext = self._contextService.getContext();
        });
        prm.add_endRequest(function(sender, args) {
            var response = args.get_response();
            var request = response.get_webRequest();            
            var context = self._contextService.getContext();
            if (request.__originalGroupContext.CurrentGroupID != context.CurrentGroupID)
            {
            ...
            else
            {
                if (request.__originalGroupContext.CurrentEntityPosition != context.CurrentEntityPosition)
                    self.onCurrentGroupPositionChanged({ current: context, previous: request.__originalGroupContext });
                if (request.__originalGroupContext.CurrentGroupCount != context.CurrentGroupCount)
                    self.onCurrentGroupCountChanged({ current: context, previous: request.__originalGroupContext });
            }
...
Sage.GroupManagerService.UI_STATE_SECTION = "GroupManagerService";

Sage.GroupManagerService.FILTER_CHANGED = "onFilterChanged";
Sage.GroupManagerService.LISTENER_HOLD_STARTED = "onListenerHoldStarted";
Sage.GroupManagerService.CURRENT_GROUP_CHANGED = "onCurrentGroupChanged";
Sage.GroupManagerService.CURRENT_GROUP_POSITION_CHANGED = "onCurrentGroupPositionChanged";
Sage.GroupManagerService.CURRENT_GROUP_COUNT_CHANGED = "onCurrentGroupCountChanged";
Sage.GroupManagerService.LOOKUPRESULTS = "LOOKUPRESULTS";

Sage.GroupManagerService.prototype.buildRequestUrl = function(options) {
    var o = options;
    var u = [];
    var p = [];
    var qp = [];      
...
Sage.GroupManagerService.prototype.getAvailableFilters = function(options, callback) {
    ...
    if (typeof options === "undefined" || options == null)
        options = { family: this._contextService.getContext().CurrentFamily, name: this._contextService.getContext().CurrentName };
     
    var sdata = { };        
    sdata.resource = "slxdata.ashx/slx/crm/-/groups";
    sdata.qparams = {
...
Sage.GroupManagerService.prototype.getLookupFields = function(options, callback) {
    if (jQuery.isFunction(options))
    {
        callback = options;
        options = null;
    ...
    if (typeof options === "undefined" || options == null)
        options = { family: this._contextService.getContext().CurrentFamily, name: this._contextService.getContext().LookoutLayoutGroupName };
...
Sage.GroupManagerService.prototype.clearDistinctValuesCache = function() {
    this._distinctValuesCache = undefined; 
}

Sage.GroupManagerService.prototype.getDistinctValuesForField = function(field, options, callback) {
    if (jQuery.isFunction(options)) {
        callback = options;
        options = null;
    }
    ...
    if (typeof options === "undefined" || options == null)
        options = {};
    if (!options.name)
        options.name = this._contextService.getContext().CurrentName;
    if (!options.family)
        options.family = this._contextService.getContext().CurrentFamily;
    //options = { family: this._contextService.getContext().CurrentFamily, name: this._contextService.getContext().CurrentName };

    var self = this;
    if (!this._distinctValuesCache) {
        this._distinctValuesCache = [];
    }
    var valueId = String.format("{0}_{1}", options.family, options.filterName);
    if (self._distinctValuesCache[valueId]) {
        callback(this._distinctValuesCache[valueId]);
        return;
    }
    var sdata = {};
    sdata.resource = "slxdata.ashx/slx/crm/-/groups";
    sdata.qparams = {
        family: options.family,
        name: options.name,
        distinctDataPath: options.dataPath,
        distinct: field,
        responsetype: "json",
...
Sage.GroupManagerService.prototype.setNewGroup = function(grpID) {
    var self = this;
    if (grpID == Sage.GroupManagerService.LOOKUPRESULTS) {
        var lookupMgr = Sage.Services.getService("GroupLookupManager");
        if (lookupMgr) {
            self.doLookup(lookupMgr.getConditionsString(), lookupMgr.withinGroup);
            return;
        }
    }
    var sdata = { };
    sdata.resource = "slxdata.ashx/slx/crm/-/groups/context";
    sdata.predicate = null;
    sdata.qparams = {
        groupid: grpID,
        responsetype: "json",
        time: new Date().getTime()
    ...
    $.ajax({
        url : self.buildRequestUrl(sdata),
        type : "POST",
        dataType: "json",
        success: OnSuccesfullGroupChanged,
        error: function(request, status, error) {
...
function OnSuccesfullGroupChanged(data) {
    var gMgr = Sage.Services.getService("GroupManagerService");
    if (gMgr) {
        var prevcontext = gMgr._contextService.getContext();
        gMgr._contextService.setContext(data);
        gMgr.onCurrentGroupChanged({ current: gMgr._contextService.getContext(), previous : prevcontext });
   }
}

Sage.GroupManagerService.prototype.doLookup = function(conditionStr, withinGroup) {
    var self = this;
    var sdata = { };
    if (typeof withinGroup === "undefined") {
        withinGroup = false;
    }
    sdata.resource = "slxdata.ashx/slx/crm/-/groups/context";
    sdata.predicate = null;
    sdata.qparams = {
        groupid : Sage.GroupManagerService.LOOKUPRESULTS,
        includegroupconditions : (withinGroup) ? "true" : "false",
...
Sage.GroupManagerService.prototype.onFilterChanged = function(args) {
    for (var i = 0 ; i < this._listeners[Sage.GroupManagerService.FILTER_CHANGED].length; i++)
    {
        var listener = this._listeners[Sage.GroupManagerService.FILTER_CHANGED][i].listener;
        var evt = this._listeners[Sage.GroupManagerService.FILTER_CHANGED][i].evt;        
        
        //if a family / name is specified for the event
        //continue if they do not match
        if (evt.family && evt.family.toLowerCase() != args.family.toLowerCase())
            continue;
        if (evt.name && evt.name.toLowerCase() != args.name.toLowerCase())
            continue;        
        ...
        {
            if (evt.timeout)
                clearTimeout(evt.timeout);                                          
            
            
            (function(l){    
                evt.timeout = setTimeout(function() {
                    if (typeof l === "function")
                        l(this, args);    
                    else if (typeof l === "object")
                        l.onFilterChanged(this, args); 
                },
                evt.hold);
            })(listener);
            
            this.onListenerHoldStarted({ held: listener, timeout: evt.timeout, args: args });
        }
    }   
};

Sage.GroupManagerService.prototype.onCurrentGroupChanged = function(args) {
    for (var i = 0 ; i < this._listeners[Sage.GroupManagerService.CURRENT_GROUP_CHANGED].length; i++)
    {
        var listener = this._listeners[Sage.GroupManagerService.CURRENT_GROUP_CHANGED][i].listener;
        if (typeof listener === "function")
            listener(this, args);    
        else if (typeof listener === "object")
            listener.onCurrentGroupChanged(this, args); 
    }
};

Sage.GroupManagerService.prototype.onCurrentGroupPositionChanged = function(args) {
    for (var i = 0 ; i < this._listeners[Sage.GroupManagerService.CURRENT_GROUP_POSITION_CHANGED].length; i++)
    {
        var listener = this._listeners[Sage.GroupManagerService.CURRENT_GROUP_POSITION_CHANGED][i].listener;
        if (typeof listener === "function")
            listener(this, args);    
        else if (typeof listener === "object")
            listener.onCurrentGroupPositionChanged(this, args); 
    }
};

Sage.GroupManagerService.prototype.onCurrentGroupCountChanged = function(args) {
    for (var i = 0 ; i < this._listeners[Sage.GroupManagerService.CURRENT_GROUP_COUNT_CHANGED].length; i++)
    {
        var listener = this._listeners[Sage.GroupManagerService.CURRENT_GROUP_COUNT_CHANGED][i].listener;
        if (typeof listener === "function")
            listener(this, args);    
        else if (typeof listener === "object")
            listener.onCurrentGroupCountChanged(this, args); 
    }
};

Sage.GroupManagerService.prototype.onListenerHoldStarted = function(args) {
    for (var i = 0 ; i < this._listeners[Sage.GroupManagerService.LISTENER_HOLD_STARTED].length; i++)
    {
        var listener = this._listeners[Sage.GroupManagerService.LISTENER_HOLD_STARTED][i].listener;
        if (typeof listener === "function")
            listener(this, args);    
        else if (typeof listener === "object")
            listener.onListenerHoldStarted(this, args); 
    }
};

Sage.GroupManagerService.prototype.requestRangeCounts = function(callback, options) {
    var self = this;

    if (this._contextService.getContext().CurrentName == null) { return; }

    if (typeof options === "undefined" || options == null)
        options = { family: this._contextService.getContext().CurrentFamily, name: this._contextService.getContext().CurrentName };

    options.keys = { family: options.family.toLowerCase(), name: options.name.toLowerCase() };
...
Sage.GroupManagerService.prototype.requestActiveFilters = function(callback, options) {
    var self = this;
    ...
    //    if (this._filters[options.keys.family] && this._filters[options.keys.family][options.keys.name]) {
    //        callback(this._filters[options.keys.family][options.keys.name]);
    //    }
    //    else {
    var sdata = {};
    sdata.resource = "slxdata.ashx/slx/crm/-/groups";
...
Sage.GroupManagerService.prototype.requestActiveFilter = function(callback, options) {
    var self = this;
    ...
    {
        callback({  
            filter: this._filters[options.keys.family][options.keys.name],
            family: options.family,
            name: options.name,
            options: options             
        });
    ...
    else
    {
        $.ajax({
            ...
            data: Ext.util.JSON.encode({
                section: Sage.GroupManagerService.UI_STATE_SECTION,
                key: "filter:" + options.keys.family + "-" + options.keys.name
            }),
            dataType: "json",
            error: function (request, status, error) {            
            },
            success: function(data, status) {  
                var filter;
                if (data && data.d) filter = Ext.util.JSON.decode(data.d);
                
                self._filters[options.keys.family] = self._filters[options.keys.family] || {};
                self._filters[options.keys.family][options.keys.name] = filter;
...
Sage.GroupManagerService.prototype.saveActiveFilter = function(family, name) {
    var self = this;    
    var key = family + "-" + name;
    
    if (this._saveFilterTimeouts[key])
        clearTimeout(this._saveFilterTimeouts[key]);
        
    this._saveFilterTimeouts[key] = setTimeout(function() { self.doSaveActiveFilter(family, name); }, 5000); 
};

Sage.GroupManagerService.prototype.doSaveActiveFilter = function(family, name) {
    var keys = {family: family.toLowerCase(), name: name.toLowerCase()};
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        ...
        data: Ext.util.JSON.encode({
            section: Sage.GroupManagerService.UI_STATE_SECTION,
            key: "filter:" + keys.family + "-" + keys.name,
            value: Ext.util.JSON.encode(this._filters[keys.family][keys.name])
        }),
        dataType: "json",
        error: function (request, status, error) {            
        },
...
Sage.GroupManagerService.prototype.setActiveFilter = function(filter, options) {
    if (typeof options === "undefined" || options == null)
        options = { family: this._contextService.getContext().CurrentFamily, name: this._contextService.getContext().CurrentName };

    options.keys = { family: options.family.toLowerCase(), name: options.name.toLowerCase() };

    this._filters[options.keys.family] = this._filters[options.keys.family] || {};
    this._filters[options.keys.family][options.keys.name] = filter;

    var caf = this._contextService.getContext().CurrentActiveFilters;
    var found = false;
    for (var i = 0; i < caf.length; i++) {
        if ((caf[i].Name == filter.filterName) && (caf[i].Entity == filter.entity)) {
            found = true;
            caf[i].Value = filter.value;
        }
    }
    if (!found) {
        this._contextService.getContext().CurrentActiveFilters.push({
            Name: filter.filterName,
            Entity: filter.entity,
            InCurrentGroupLayout: true,
            FilterId: Sage.FiltersToJSId(filter.entity) + "_" + Sage.FiltersToJSId(filter.filterName),
            Value: filter.value
        });
    }
    if (filter.filterType == 'clear') caf = [];
    this._contextService.getContext().CurrentActiveFilters = caf;
    this.saveActiveFilter(options.keys.family, options.keys.name);
    this.onFilterChanged({ filter: filter, family: options.family, name: options.name, options: options });
};

Sage.GroupManagerService.prototype.addListener = function(event, listener, options) {
    this._listeners[event] = this._listeners[event] || [];  
...
Sage.GroupManagerService.prototype.removeListener = function(event, listener) {
    this._listeners[event] = this._listeners[event] || [];
...
Sage.GroupManagerService.prototype.findGroupInfoList = function() {
    if (typeof (groupInfoList) != "undefined") {
        return groupInfoList;
    }
}

Sage.GroupManagerService.prototype.findGroupInfoById = function(groupId) {
    var list = this.findGroupInfoList();
    if (list) {
        if (typeof(list.groupInfos) != "undefined") {
            for (var i = 0; i < list.groupInfos.length; i++) {
                if (list.groupInfos[i].groupID == groupId)
...
Sage.FilterManager = function(options) {
    ...
    Sage.FilterManager.EditFiltersString = TaskPaneResources.EditFilters;  //'Edit Filters';     
...
Sage.SaveHiddenFilters = function(hiddenFilters, oncomplete) {
    ...
    if (tabPanel) {
        ...
        var GroupContext = Sage.Services.getService("ClientGroupContext");
        if (GroupContext) {
            groupkey = GroupContext.getContext().CurrentGroupID.toLowerCase(); 
...
Sage.FilterManager.prototype.loadDistinctFilterValues = function(parent, container, data, filter, alias) {
    ...
    for (var i = 0; i < data.items.length; i++) {
        ...
        item.entity = data.entityName || data.family;
...
Sage.FilterActivityManager.prototype.getDistinctValuesForField = function(field, options, success, fail) {
    ...
    this.requestRangeCounts(function(data) {
        ...
        success(d);
    }, { type: options.family, field: field, filterName: options.filterName }, fail);
};
...
Sage.FilterActivityManager.prototype.requestRangeCounts = function(success, options, fail) {
    var self = this;
    var family = options.type ? options.type.toLowerCase() : "";
    
    // --- Retrieve the group name ----
    var tabPanel = Ext.ComponentMgr.get('activity_groups_tabs');
        if (typeof tabPanel === "undefined")
            return null;
    var resource = tabPanel.activeTab.connections.list.resource;
    var group = resource.substring(resource.lastIndexOf("/")+1 , resource.length)
    group = (group == '') ? "all" : group;   
    //if the family is not the current resrouce then use the 'all' query
    if (resource.indexOf(family) == -1) group = 'all';
    // ---------
    
    var filterName = options.filterName ? options.filterName.toLowerCase() : "";
    if (self.RangeCountCache) {
        for (var i = 0; i < self.RangeCountCache.length; i++) {
            if ((self.RangeCountCache[i].FilterEntity.toLowerCase() == family) &&
                (self.RangeCountCache[i].FilterName.toLowerCase() == filterName) &&
                (self.RangeCountCache[i].Counts != null)) {
    ...
    var sdata = {};
    sdata.resource = String.format("slxdata.ashx/slx/crm/-/activities/{0}count/{1}", family, group);
    sdata.qparams = {
        responsetype: "json",
