...
LookupControl.prototype.refresh = function(options) {
    /* can refresh properties on the control from server side values */
    this._options = options;
    this._id = options.id;
    this._clientId = options.clientId;
    this._gridId = options.gridId;
...
LookupControl.prototype.setGridParameters = function(seedValue, reload) {
    ...
    if (seedQuery)
        ...
        if (!this._initializeLookup && !this.hasDialog()) /* we only want to do this on the first show */
...
LookupControl.prototype.wireUpGridEvents = function() {        
    ...
    this.getGrid().getNativeGrid().on('rowdblclick', function(nativeGrid, rowIndex, evt) {
            /* will not work with the buffered store */
