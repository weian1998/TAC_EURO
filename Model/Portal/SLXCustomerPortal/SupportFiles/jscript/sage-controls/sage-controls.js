...
AddrTypeComboBox=function(options){var self=this;var o=options||{};o.mode="local";o.displayField="addrTypeDisplay";o.valueField="addressType";o.editable=false;o.triggerAction="all";var rt=Ext.data.Record.create([{name:"addressType",mapping:1},{name:"addrTypeDisplay",mapping:2}]);var typestore=new Ext.data.Store({reader:new Ext.data.ArrayReader({idIndex:0},rt)});var typeData=[[1,"Billing",addressrsc.AddressControl_EditPanel_AddrType_Billing],[2,"Shipping",addressrsc.AddressControl_EditPanel_AddrType_Shipping],[3,"Billing & Shipping",addressrsc.AddressControl_EditPanel_AddrType_BillingShipping],[4,"Correspondence",addressrsc.AddressControl_EditPanel_AddrType_Correspondence],[5,"Home",addressrsc.AddressControl_EditPanel_AddrType_Home],[6,"Office",addressrsc.AddressControl_EditPanel_AddrType_Office],[7,"Other",addressrsc.AddressControl_EditPanel_AddrType_Other],[8,"Unknown",addressrsc.AddressControl_EditPanel_AddrType_Unknown]];typestore.loadData(typeData);this.store=o.store=typestore;AddrTypeComboBox.superclass.constructor.call(this,o);}
Ext.extend(AddrTypeComboBox,Ext.form.ComboBox,{initComponent:function(){AddrTypeComboBox.superclass.initComponent.call(this);this.bind();},bind:function(){},unbind:function(){},setValue:function(v){AddrTypeComboBox.superclass.setValue.call(this,v);},getValue:function(){return AddrTypeComboBox.superclass.getValue.call(this);}});Ext.reg('addrtypecombo',AddrTypeComboBox);PickListComboBox=function(options){var self=this;var o=options||{};this.pickListRequestUrl=o.pickListRequestUrl||"slxdata.ashx/slx/crm/-/picklists/find";this.pickListName=o.pickListName;var m={text:"text",id:"itemId",code:"code"};this.store=o.store=new Ext.data.Store({baseParams:{name:o.pickListName,sort:o.sort},proxy:new Ext.data.HttpProxy({url:String.format(this.pickListRequestUrl,escape(this.pickListName)),method:"GET"}),reader:new Ext.data.JsonReader({root:"items",id:"id",fields:[{name:"text"},{name:"value",convert:function(v,rec){return m[o.storageMode]?rec[m[o.storageMode]]:rec["text"];}}]}),listeners:{load:function(){if(typeof idComboBoxItems==="function"){idComboBoxItems();}}}});o.mode="remote";o.displayField="text";o.valueField="value";o.triggerAction="all";o.minChars=(typeof o.minChars==="number")?o.minChars:4;o.typeAhead=(typeof o.typeAhead==="boolean")?o.typeAhead:false;o.editable=(typeof o.editable==="boolean")?o.editable:!o.mustExistInList;o.sort=(typeof o.sort==="boolean")?o.sort:true;PickListComboBox.superclass.constructor.call(this,o);}
...
{this.clientId=clientId;this.displayID=clientId+"_displayText";this.dlgDivID=clientId+"_outerDiv";this.returnValueId=clientId+"_resultText";;this.desc1value=desc1;this.desc2value=desc2;this.desc3value=desc3;this.cityvalue=city;this.statevalue=state;this.zipvalue=zip;this.countryvalue=country;this.countyvalue=county;this.salutationvalue=salutation;this.description=description;this.isPrimary=primary;this.isMailing=mailing;this.typevalue=addrtype;this.AutoPostBack=autoPostBack;this.panel=null;}
...
this.typevalue=document.getElementById(this.clientId+"_AddrType").value;var returnValue=document.getElementById(this.returnValueId);returnValue.value=this.desc1value+"|"+this.desc2value+"|"+this.desc3value+"|"+this.cityvalue+"|"+this.statevalue+"|"+this.zipvalue+"|"+this.countryvalue+"|"+this.salutationvalue+'|'+this.description+'|'+this.isPrimary+'|'+this.isMailing+'|'+this.typevalue;this.FormatAddress(this.displayID);}
...
return false;}
return true;};Sage.UI.Numeric.prototype.changed=function(e){this.justChanged=true;}
Sage.UI.Numeric.prototype.validate=function(){this.formatNumber();if(this.justChanged&&this.autoPostBack){this.justChanged=false;if(Sys){Sys.WebForms.PageRequestManager.getInstance()._doPostBack(this.controlId,null);}
else{document.forms(0).submit();}}
this.justChanged=false;};Sage.UI.Numeric.prototype.formatNumber=function(){var control=dojo.byId(this.controlId);var value=control.value;if(value===''){this.currValue=0;control.value=0;return;}
...
Sage.SyncExec.call(handler);return false;}};DependencyLookup.prototype.Show=function(){if(this.CanShow()){if(this.panel==null){var lookup=document.getElementById(this.PanelId);lookup.style.display="block";this.panel=new YAHOO.widget.Panel(this.PanelId,{visible:false,width:this.Size,fixedcenter:true,constraintoviewport:true,underlay:"shadow",draggable:true,iframe:true,modal:false});this.panel.render();if((this.CurrentIndex==0)||(this.LookupControls[i]!=undefined)){this.Init();}
...
this.panel.show();}};DependencyLookup.prototype.AddControl=function(baseId,listId,textId,type,displayProperty,seedProperty){var dependCtrl=new dependControl(baseId,listId,textId,type,displayProperty,seedProperty);this.LookupControls[this.CurrentIndex]=dependCtrl;this.CurrentIndex++;};DependencyLookup.prototype.AddFilters=function(FilterProp,FilterValue){var dependCtrl=new dependControl(listId,textId,type,displayProperty,seedProperty);this.LookupControls[this.CurrentIndex]=dependCtrl;this.CurrentIndex++;};DependencyLookup.prototype.SelectionChanged=function(index){if((index+1)<this.CurrentIndex){this.LookupControls[index+1].LoadList(this.GetSeeds(index+1));for(var i=index+2;i<this.CurrentIndex;i++){this.LookupControls[i].ClearList();}}};DependencyLookup.prototype.Ok=function(){this.panel.hide();for(var i=0;i<this.CurrentIndex;i++){var text=document.getElementById(this.LookupControls[i].TextId);var list=document.getElementById(this.LookupControls[i].ListId);if((list.selectedIndex!=undefined)&&(list.selectedIndex!=-1)){text.value=list.options[list.selectedIndex].text;}
else{text.value="";}
...
{this.hide();return;}
this.show();};separator.render=function(td){this.td=td;if(this.el)td.appendChild(this.el);if(this.hidden)
this.td.style.display="none";};items.push(separator);}
items.push({slxUpdate:function(l){if((l.views.list&&l.connections.list)&&((l.views.summary&&l.connections.summary)||(l.views.timeline&&l.connection.timeline)))
...
self.switchView("list");}}}});items.push({slxUpdate:function(l){if(l.views.summary&&l.connections.summary)
...
items.push({slxUpdate:function(l){this.suspendEvents();this.toggle(l.activeView=="timeline");this.resumeEvents();},id:this.makeId("button","timeline"),text:this.timelineText,listeners:{},enableToggle:true,pressed:(this.activeView=="timeline"),toggleGroup:this.makeId("button","toggle","group"),listeners:{toggle:{fn:function(b,t){if(t)
self.switchView("timeline");}}}});if(this.helpUrl)
items.push({icon:"ImageResource.axd?scope=global&type=Global_Images&key=Help_16x16",tooltip:Sage.SalesLogix.Controls.Resources.ListPanel.Help,text:false,cls:"x-btn-text-icon x-btn-icon-only",handler:function(){window.open(self.helpUrl,'MCWebHelp');}});this.tbar=new Sage.SalesLogix.Controls.ListPanelToolbar(items);};Sage.SalesLogix.Controls.ListPanel.prototype.switchView=function(view,forceLayout){if(this.activeView===view)
...
items[i].slxUpdate.call(items[i],this);};Sage.SalesLogix.Controls.ListPanel.prototype.initManagers=function(){for(var k in this.managers)
...
return this.views.list.getSelectionModel().getSelections();};Sage.SalesLogix.Controls.ListPanel.prototype.getSelectionInfo=function(){if(this.activeView!="list")
return[];var recordCount=this.views.list.store.reader.jsonData.count;var selectionCount=this.getTotalSelectionCount();var selectionKey=this.id;var mode='selection';var selections=[];var ranges=[];var keyField=this.views.list.view.ds.converter.meta.keyfield;var sortDirection=this.views.list.view.ds.sortInfo?this.views.list.view.ds.sortInfo.direction:'';var sortField=this.views.list.view.ds.sortInfo?this.views.list.view.ds.sortInfo.field:'';if(recordCount==selectionCount)
{if(selectionCount>0)
{mode='selectAll'}}
else
...
{var item={header:svc.CurrentName,align:'left',sortable:false,dataIndex:svc.getContext().CurrentTableKeyField,renderer:this.summaryRenderer};}
...
id=svc.getContext().CurrentTableKeyField;columns.push({name:id,sortType:"string"});columns.push({name:'SLXRN',sortType:'int'});return{meta:this.meta,recordType:columns};},summaryRenderer:function(value,meta,record,rowIndex,columnIndex,store){if(!value)
return"&nbsp;";if(value.length!=12)
return value;var el=value+"_"+Sage.SalesLogix.Controls.GroupSummaryMetaConverter.renderedCount++;store.manager.show({id:value,record:record},el,true);return String.format('<div id="{0}" class="list-view-summary-item"><div class="loading-indicator">{1}</div></div>',el,Sage.SalesLogix.Controls.Resources.ListPanel.LoadingMsg);}});Sage.SalesLogix.Controls.GroupSummaryMetaConverter.renderedCount=0;Ext.reg("groupsummarymetaconverter",Sage.SalesLogix.Controls.GroupSummaryMetaConverter);Sage.UserTypeToDisplayName=function(value){var userTypes={'W':Sage.SalesLogix.Controls.Resources.ListPanel.Administrator,'P':Sage.SalesLogix.Controls.Resources.ListPanel.Template,'M':Sage.SalesLogix.Controls.Resources.ListPanel.Remote,'T':Sage.SalesLogix.Controls.Resources.ListPanel.WebOnly,'R':Sage.SalesLogix.Controls.Resources.ListPanel.Retired,'C':Sage.SalesLogix.Controls.Resources.ListPanel.Concurrent,'V':Sage.SalesLogix.Controls.Resources.ListPanel.WebViewer,'N':Sage.SalesLogix.Controls.Resources.ListPanel.Network,'A':Sage.SalesLogix.Controls.Resources.ListPanel.AddOnUser};return(userTypes[value]?Ext.util.Format.htmlEncode(userTypes[value]):value);};Sage.OwnerTypeToDisplayName=function(value){var ownerTypes={'G':Sage.SalesLogix.Controls.Resources.ListPanel.Team,'D':Sage.SalesLogix.Controls.Resources.ListPanel.Department,'S':Sage.SalesLogix.Controls.Resources.ListPanel.System,'U':Sage.SalesLogix.Controls.Resources.ListPanel.User};return(ownerTypes[value]?Ext.util.Format.htmlEncode(ownerTypes[value]):value);};Sage.SalesLogix.Controls.GroupMetaConverter=Ext.extend(Sage.SalesLogix.Controls.MetaConverter,{toBoolean:function(v){if(typeof v==="boolean")
...
{case"SalesLogix.PickList":case"SalesLogix.MRCodePickList":case"System.String":value=value.replace(new RegExp("\\\\","g"),"\\\\").replace(new RegExp("\\'","g"),"''");if(quote)
...
this.getGrid().getNativeGrid().getStore().reload();};LookupControl.prototype.initGrid=function(seedValue,reload){if(this.getGrid()&&this.getGrid().hasStaticLayout()){var isGridInitialized=(this.getGrid().getNativeGrid())?true:false;var reload=(!isGridInitialized)?false:reload;this.setGridParameters(seedValue,reload);this.getGrid().clearMetaData();if(!isGridInitialized){this.getGrid().init();this.wireUpGridEvents();}}
this.getGrid().getNativeGrid().getTopToolbar().hide();};LookupControl.prototype.wireUpGridEvents=function(){var self=this;this.getGrid().getNativeGrid().on('rowdblclick',function(nativeGrid,rowIndex,evt){var view=nativeGrid.getView();var store=nativeGrid.getStore();var selected=store.getAt(rowIndex);if(!selected)
...
else{var grid=self.getGrid();grid.getNativeGrid().getStore().setDefaultSort(property,'ASC');delete grid.getConnections()["data"].parameters["where"];if(!self._overrideSeedOnSearch){var seedQuery=self.getSeedQueryExpression();if(seedQuery)
grid.getConnections()["data"].parameters["where"]=seedQuery;}
grid.getNativeGrid().getStore().load();}}});dialog.on('hide',function(){self.fireEvent('close',this);},dialog);dialog.on('show',function(){var context=dialog.getEl().dom;if(!this._initialized){searchButton.render($(".lookup-search",context).get(0));var select=$(".lookup-properties select",context).addOption(self._propertyPairs).change(function(e){$("option:selected",this).each(function(){var property=self._propertyLookup[$(this).val()];if(property){var operators={};if(LookupControl.__operators[property.type])
...
operators[LookupControl.__defaultOperators[i].value]=LookupControlResources[LookupControl.__defaultOperators[i].name];var select=$(".lookup-operators select",context).get(0);$(select).removeOption(/./).addOption(operators);select.selectedIndex=0;if(property.type=="SalesLogix.PickList"){$(".lookup-values input",context).val("").hide();$(".lookup-values select",context).removeOption(/./).show();$.ajax({url:String.format("slxdata.ashx/slx/crm/-/picklists/{0}",property.pickListName),data:{format:"json"},dataType:"json",success:function(data){var options={};for(var i=0;i<data.items.length;i++){var name=data.items[i].text;var value=data.items[i].itemId||data.items[i].text;options[value]=name;}
$(".lookup-values select",context).addOption(options);},error:function(request,status,error){}});}
else if(property.type=="SalesLogix.MRCodePickList"){$(".lookup-values input",context).val("").hide();$(".lookup-values select",context).removeOption(/./).show();$.ajax({url:String.format("slxdata.ashx/slx/crm/-/CodePickLists({0})",property.pickListName),data:{format:"json"},dataType:"json",success:function(data){var options={};for(var i=0;i<data.items.length;i++){var name=data.items[i].displayValue;var value=data.items[i].pickListItemPath||data.items[i].displayValue;options[value]=name;}
$(".lookup-values select",context).addOption(options);},error:function(request,status,error){}});}
else if(property.type=="Sage.Entity.Interfaces.OwnerType"){$(".lookup-values input",context).val("").hide();$(".lookup-values select",context).removeOption(/./).show();$.ajax({type:"GET",contentType:"application/json",url:"slxdata.ashx/slx/crm/-/ownertypes",data:{format:"json"},dataType:"json",success:function(data){var options={};for(var i=0;i<data.length;i++){if(data[i].code!="S"){var name=data[i].name;var value="'"+data[i].code+"'";options[value]=name;}}
$(".lookup-values select",context).addOption(options);},error:function(request,status,error){}});}
else if(property.type=="System.Boolean"){$(".lookup-values input",context).val("").hide();$(".lookup-values select",context).removeOption(/./),addOption({"true":"true","false":"false"}).show();}
else{$(".lookup-values select",context).removeOption(/./).hide();$(".lookup-values input",context).show();}}});}).get(0);if(select&&select.options.length>0){select.selectedIndex=0;$(select).change();}
$(".lookup-values input, .lookup-values select",context).bind("keypress",function(e){if(e.which==13){if(document.createEvent){var button=searchButton.getEl().dom;var evt=document.createEvent("MouseEvents");evt.initMouseEvent("click",true,true,window,0,0,0,0,0,false,false,false,false,0,null);button.dispatchEvent(evt);}
else{$("#"+searchButton.getEl().dom.id).click();}
...
function NamePanel_NameChanged(){if(event.srcElement.value!==this.CombineName()){this.TextChanged=true;}}
function NamePanel_CombineName(){var name=((this.prefix=="")?"":this.prefix+' ');name+=((this.first=="")?"":this.first+' ');name+=((this.middle=="")?"":this.middle+' ');name+=((this.last=="")?"":this.last+' ');name+=((this.suffix=="")?"":this.suffix);name=this.Trim(name);return name;}
function NamePanel_FormatName(){var name=this.CombineName();var elem=document.getElementById(this.displayID);elem.value=name;this.TextChanged=false;if(this.AutoPostBack){if(Sys){Sys.WebForms.PageRequestManager.getInstance()._doPostBack(this.displayID,null);}
else{document.forms(0).submit();}}}
function NamePanel_ParseName(){if(!this.TextChanged){return;}
...
NamePanel.prototype.FormatName=NamePanel_FormatName;NamePanel.prototype.CombineName=NamePanel_CombineName;NamePanel.prototype.Show=NamePanel_Show;NamePanel.prototype.Cancel=NamePanel_Cancel;NamePanel.prototype.Ok=NamePanel_Ok;NamePanel.prototype.HandleKeyEvent=NamePanel_HandleKeyEvent;NamePanel.prototype.NameChanged=NamePanel_NameChanged;NamePanel.prototype.ParseName=NamePanel_ParseName;NamePanel.prototype.Trim=NamePanel_Trim;

function OwnerControl(options){options=options||{};this._dialog=null;this._options=options;this._clientId=options.clientId;this._textClientId=options.textClientId;this._valueClientId=options.valueClientId;this._autoPostBack=options.autoPostBack;this._identifyNodes=true;this._result=false;this._typeAheadText=false;this._typeAheadTimeout=false;this._helpLink=options.helpLink;this._multiSelect=options.multiSelect||false;this._types=options.types||"U,T,S";OwnerControl.__instances[this._clientId]=this;OwnerControl.__initRequestManagerEvents();this.addEvents('open','close','change');};Ext.extend(OwnerControl,Ext.util.Observable);OwnerControl.__instances={};OwnerControl.__requestManagerEventsInitialized=false;OwnerControl.__initRequestManagerEvents=function(){if(OwnerControl.__requestManagerEventsInitialized)
return;var contains=function(a,b){if(!a||!b)
return false;else
return a.contains?(a!=b&&a.contains(b)):(!!(a.compareDocumentPosition(b)&16));};var prm=Sys.WebForms.PageRequestManager.getInstance();prm.add_pageLoaded(function(sender,args){var panels=args.get_panelsUpdated();if(panels){for(var i=0;i<panels.length;i++){for(var id in OwnerControl.__instances)
if(contains(panels[i],document.getElementById(id))){var instance=OwnerControl.__instances[id];instance.initContext();}}}});OwnerControl.__requestManagerEventsInitialized=true;};OwnerControl.prototype.init=function(){this.initContext();};OwnerControl.prototype.initContext=function(){this._context=document.getElementById(this._clientId);};OwnerControl.prototype.identifyNode=function(node){if(!this._identifyNodes||node._wasIdentified)
return;for(var i=0;i<node.childNodes.length;i++){var child=node.childNodes[i];var el=child.getUI().getAnchor();$(el).attr("id",(this._clientId+"_"+child.id));}
node._wasIdentified=true;};OwnerControl.prototype.createDialog=function(){function getChildren(ownertypes){var children=[];if(ownertypes.indexOf('U')>-1)
children.push({id:"user",text:OwnerControlResources.OwnerControl_OwnerType_User,cls:"owner-type-user-container",singleClickExpand:true,expanded:false,listeners:{"expand":function(node){self.identifyNode(node);}}});if(ownertypes.indexOf('T')>-1)
children.push({id:"team",text:OwnerControlResources.OwnerControl_OwnerType_Team,cls:"owner-type-team-container",singleClickExpand:true,listeners:{"expand":function(node){self.identifyNode(node);}}});if(ownertypes.indexOf('D')>-1)
children.push({id:"department",text:"Department",cls:"owner-type-department-container",singleClickExpand:true,listeners:{"expand":function(node){self.identifyNode(node);}}});if(ownertypes.indexOf('S')>-1)
children.push({id:"system",text:OwnerControlResources.OwnerControl_OwnerType_System,cls:"owner-type-system-container",singleClickExpand:true,listeners:{"expand":function(node){self.identifyNode(node);}}});return children;}
var self=this;var root=new Ext.tree.AsyncTreeNode({text:"Root",id:"root",expanded:true,listeners:{"expand":function(node){self.identifyNode(node);}},children:getChildren(this._types)});var loader=new Ext.tree.TreeLoader({url:"slxdata.ashx/slx/crm/-/owners/{0}",requestMethod:"GET"});loader.on("beforeload",function(treeLoader,node){treeLoader.formatUrl=treeLoader.formatUrl||treeLoader.url;treeLoader.url=String.format(treeLoader.formatUrl,node.id);});var tree=new Ext.tree.TreePanel({id:this._clientId+"_tree",root:root,rootVisible:false,containerScroll:true,autoScroll:true,animate:false,frame:false,stateful:false,loader:loader,selModel:(this._multiSelect)?new Ext.tree.MultiSelectionModel():new Ext.tree.DefaultSelectionModel(),border:false});var panel=new Ext.Panel({id:this._clientId+"_container",layout:"fit",border:false,stateful:false,items:tree});var dialog=new Ext.Window({id:this._clientId+"_window",title:OwnerControlResources.OwnerControl_Header,cls:"lookup-dialog",layout:"border",closeAction:"hide",plain:true,height:400,width:300,stateful:false,constrain:true,modal:true,items:[{region:"center",margins:"0 0 0 0",border:false,layout:"fit",id:this._clientId+"_layout_center",items:panel}],buttonAlign:"right",buttons:[{id:this._clientId+"_ok",text:GetResourceValue(OwnerControlResources.OwnerControl_Button_Ok,"OK"),handler:function(){if(self._multiSelect){var nodes=tree.getSelectionModel().getSelectedNodes();var ids=[];var texts=[];for(var i=0;i<nodes.length;i++){ids.push(nodes[i].id);texts.push(nodes[i].text);}
self.close();self.setResult(ids.join(','),texts.join(','));return;}
var node=tree.getSelectionModel().getSelectedNode();if(!node||!node.isLeaf()){Ext.Msg.alert("",OwnerControlResources.SlxOwnerControl_OwnerType_NoRecordSelected);return;}
...
window.open(self._helpLink.url,(self._helpLink.target||"help"));}}]});dialog.on("show",function(win){$(document).bind("keypress.OwnerControl",function(evt){var key=evt.charCode||evt.keyCode;var text=String.fromCharCode(key);if(self._typeAheadTimeout)
...
self._typeAheadText=text;var node;if(self._multiSelect){node=root;}else{node=tree.getSelectionModel().getSelectedNode();if(node){if(node.parentNode&&(node.isLeaf()||!node.isExpanded()))
node=node.parentNode;}else{node=root;}}
var match=self.search(node,new RegExp("^"+text,"i"));if(match){match.select();return false;}});if(typeof idLookup!="undefined")idLookup("lookup-dialog");});dialog.on("hide",function(win){$(document).unbind("keypress.OwnerControl");});tree.on("dblclick",function(node,evt){if(!node||!node.isLeaf())
return;self.close();self.setResult(node.id,node.text);});this._dialog=dialog;this._tree=tree;};OwnerControl.prototype.search=function(node,expr){if(node){if(node.id!="root"&&expr.test(node.text))
return node;if(node.isExpanded()){for(var i=0;i<node.childNodes.length;i++){var child=node.childNodes[i];var match=false;if(child.isLeaf()){if(expr.test(child.text))
return child;}
...
__doPostBack(this._clientId,'');};OwnerControl.prototype.invokeChangeEvent=function(el){if(document.createEvent){var evt=document.createEvent('HTMLEvents');evt.initEvent('change',true,true);el.dispatchEvent(evt);}
else{el.fireEvent('onchange');}};function OwnerPanel(clientId,autoPostBack){this.ClientId=clientId;this.OwnerDivId=clientId+"_outerDiv";this.OwnerTypeId=clientId+"_OwnerType";this.OwnerTextId=clientId+"_LookupText";this.OwnerValueId=clientId+"_LookupResult";this.ResultDivId=clientId+"_Results";this.OwnerListId=clientId+"_OwnerList";this.AutoPostBack=autoPostBack;this.LookupValue="";this.DisplayValue="";this.onChange=new YAHOO.util.CustomEvent("change",this);this.panel=null;}
function OwnerPanel_CanShow(){var inPostBack=false;if(Sys){var prm=Sys.WebForms.PageRequestManager.getInstance();inPostBack=prm.get_isInAsyncPostBack();}
if(!inPostBack){return true;}
else{var id=this.ClientId+"_obj";var handler=function(){window[id].Show('');}
Sage.SyncExec.call(handler);return false;}}
function OwnerPanel_Show(){if(this.CanShow()){if((this.panel==null)||(this.panel.element.parentNode==null)){var lookup=document.getElementById(this.OwnerDivId);lookup.style.display="block";this.panel=new YAHOO.widget.Panel(this.OwnerDivId,{visible:false,width:"305px",fixedcenter:true,constraintoviewport:true,underlay:"shadow",draggable:true,modal:false});this.panel.render();}
this.panel.show();var ownerType=document.getElementById(this.OwnerTypeId);var list=document.getElementById(this.OwnerListId);if(list.options.length<1){var type=ownerType.selectedIndex;if(type==-1){type=0;}
...
xmlhttp.open("GET",vURL,false);xmlhttp.send(null);var results=xmlhttp.responseText;var items=results.split("|");for(var i=0;i<items.length;i++){if(items[i]=="")continue;var parts=items[i].split("*");var oOption=document.createElement("OPTION");list.options.add(oOption);if(parts[0].charAt(0)=='@'){parts[0]=parts[0].substr(1);oOption.selected=true;}
oOption.innerHTML=parts[1];oOption.value=parts[0];}}}}
function OwnerPanel_PerformLookup(){var ownerType=document.getElementById(this.OwnerTypeId);var list=document.getElementById(this.OwnerListId);for(var i=list.options.length-1;i>=0;i--){list.options[i]=null;}
var type=ownerType.selectedIndex;if(type==-1){type=0;}
var vURL="SLXOwnerListHandler.aspx?ownertype="+type;if(typeof(xmlhttp)=="undefined"){xmlhttp=YAHOO.util.Connect.createXhrObject().conn;}
xmlhttp.open("GET",vURL,false);xmlhttp.send(null);var results=xmlhttp.responseText;if(results=="NOTAUTHENTICATED"){window.location.reload(true);return;}
var items=results.split("|");for(var i=0;i<items.length;i++){if(items[i]=="")continue;var parts=items[i].split("*");var oOption=document.createElement("OPTION");list.options.add(oOption);if(parts[0].charAt(0)=='@'){parts[0]=parts[0].substr(1);oOption.selected=true;}
oOption.innerHTML=parts[1];oOption.value=parts[0];}}
function OwnerPanel_Close(){this.panel.hide();}
function OwnerPanel_Ok(){this.panel.hide();var list=document.getElementById(this.OwnerListId);try{this.LookupValue=list.options[list.selectedIndex].value;}
catch(e){Ext.Msg.alert("",OwnerControlResources.SlxOwnerControl_OwnerType_NoRecordSelected);return false;}
this.DisplayValue=list.options[list.selectedIndex].innerHTML;var ownerText=document.getElementById(this.OwnerTextId);var ownerValue=document.getElementById(this.OwnerValueId);ownerText.value=this.DisplayValue;ownerValue.value=this.LookupValue;this.onChange.fire(this);this.InvokeChangeEvent(ownerText);if(this.AutoPostBack){if(Sys){Sys.WebForms.PageRequestManager.getInstance()._doPostBack(this.ClientId,null);}
else{document.forms(0).submit();}}}
function OwnerPanel_getVal(){return this.LookupValue;}
function OwnerPanel_setVal(key,display){this.LookupValue=key;this.DisplayValue=display;var lookupText=document.getElementById(this.LookupTextId);lookupText.value=this.DisplayValue;}
function OwnerPanel_GetOwnerTypeParam(){var ownerType=document.getElementById(this.OwnerTypeId);var arg=-1;if(ownerType.options.length>0){arg=ownerType.selectedIndex;}
return arg;}
function OwnerPanel_InvokeChangeEvent(cntrl){if(document.createEvent){var evObj=document.createEvent('HTMLEvents');evObj.initEvent('change',true,true);cntrl.dispatchEvent(evObj);}
else{cntrl.fireEvent('onchange');}}
...
delete $(store).data("filter").selected[value];var svc=this.GetManagerService();var values=[];for(var key in $(store).data("filter").selected)values.push(key);if(svc){svc.setActiveFilter({filterName:filterDef.Name,entity:filterDef.EntityName,field:filterDef.FieldName,value:values,filterType:'Distinct',tableName:filterDef.TableName,property:filterDef.PropertyName});}}
...
for(var i=0;i<data.items.length;i++){var item=data.items[i];item.alias=alias;item.display=(i<this._expandCount);item.index=i;item.selected=false;item.count=item.TOTAL==-1?Sage.FilterManager.UnknownCount:item.TOTAL;item.displayName=GetDisplayName(data,item);item.objname=self._id;item.entity=data.entityName||data.family;item.displayNameId=Sage.FiltersToJSId(item.VALUE);FixNullBlank(item);item.valueWithSingleFixed=(typeof item.VALUE)=="string"?item.VALUE.replace(/'/g,"\\\'"):item.VALUE;var cssclass=(csssearchstring.indexOf('|'+item.VALUE.toString().toLowerCase()+'|')>-1)?"display-none":"";html.push(String.format(Sage.FilterManager.valueItemFormat,item.entity,item.alias,item.index,item.VALUE,(item.selected)?"checked=\"checked\"":"",item.objname,Sage.FiltersToJSId(item.entity),Sage.FiltersToJSId(item.alias),item.valueWithSingleFixed,item.displayName,item.displayNameId,item.count,cssclass));}
...
d.items.sort();d.items.reverse();d.count=d.items.length;d.total_count=d.items.length;d.distinctField=field;success(d);},{type:options.family,field:field,filterName:options.filterName},fail);};function GetFilterDisplayName(val,defname){if(typeof(UserNameLookup)!="undefined"){if(UserNameLookup[val])
