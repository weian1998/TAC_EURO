function SLXDataGrid(ElementID) {
    if (ElementID) {
        ...
        this.gridElement = dojo.byId(this.ID);

        // call init only once the element is available
        dojo.ready(dojo.hitch(this, this.init));
    }

}

SLXDataGrid.prototype.onrendercomplete = function() {
}

SLXDataGrid.prototype.onrowselect = function(row) {
}

SLXDataGrid.prototype.onrowdeselect = function(row) {
}

SLXDataGrid.prototype.onrowdelete = function(row) {
}

SLXDataGrid.prototype.onrowadd = function(index) {
}
...
SLXDataGrid.prototype.attachEvents = function () {
    ...
    for (var i = 0; i < this.tHeadRow.cells.length; i++) {
        ...
        dojo.connect(c, 'onmousemove', this, this.isHot, true);
        dojo.connect(c, 'onmousedown', this, this.startResize, true);
        dojo.connect(c, 'onmouseup', this, this.endResize, true);
    }
    ...
    for (var i = 1; i < this.gridElement.rows.length; i++) {
        ...
        r.vAlign = 'top';
        dojo.connect(r, 'onclick', this, this.SelectRow, true);
    }
...
SLXDataGrid.prototype.SelectRow = function (ev) {
    ...
    if (this.selRow) {
        if (this.selRow == rowIdx) {
            ...
            this.onrowdeselect(this.selRow);
            //this.selRow = null;
        } else {
            this.normalizeRow(this.selRow);
            this.selRow = rowIdx;
    ...
    if (typeof (iIndex) != 'undefined') {
        if (!isNaN(iIndex)) {
            if ((iIndex > 0) && (iIndex <= this.gridElement.rows.length)) {
                r = this.gridElement.insertRow(iIndex);
            } else {
                r = this.gridElement.insertRow(this.gridElement.rows.length);
            }
        } else {
            r = this.gridElement.insertRow(this.gridElement.rows.length);
        }
